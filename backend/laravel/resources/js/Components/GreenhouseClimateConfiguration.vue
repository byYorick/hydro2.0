...
const runtimeFields: NumericField[] = [
  { key: 'emergencyIntervalSeconds', label: 'Аварийный интервал (сек)', helpKey: 'climate.emergencyIntervalSeconds', fallback: 60, min: 10, max: 3600 },
  { key: 'minCommandIntervalSeconds', label: 'Мин. интервал команд (сек)', helpKey: 'climate.minCommandIntervalSeconds', fallback: 300, min: 0, max: 3600 },
  { key: 'positionDeadbandPercent', label: 'Мёртвая зона (%)', helpKey: 'climate.positionDeadbandPercent', fallback: 5, min: 0, max: 50 },
  { key: 'minSafeOpenPercent', label: 'Мин. безопасное открытие (%)', helpKey: 'climate.minSafeOpenPercent', fallback: 15, min: 0, max: 100 },
  { key: 'fallbackOpenPercent', label: 'Аварийное открытие fallback (%)', helpKey: 'climate.fallbackOpenPercent', fallback: 5, min: 0, max: 100 },
  { key: 'weatherStaleMaxOpenPercent', label: 'Лимит при устаревшей погоде (%)', helpKey: 'climate.weatherStaleMaxOpenPercent', fallback: 20, min: 0, max: 100 },
  { key: 'emergencyOpenPercent', label: 'Аварийное открытие (%)', helpKey: 'climate.emergencyOpenPercent', fallback: 100, min: 0, max: 100 },
  { key: 'daylightLuxThreshold', label: 'Порог дневного света (люкс)', helpKey: 'climate.daylightLuxThreshold', fallback: 15000, min: 0, max: 200000 },
  { key: 'sensorFreshnessSeconds', label: 'Свежесть сенсора (сек)', helpKey: 'climate.sensorFreshnessSeconds', fallback: 1200, min: 30, max: 86400 },
]

const ventPositionFields: NumericField[] = [
  { key: 'nightBaseOpenPercent', label: 'Ночное базовое (%)', helpKey: 'climate.nightBaseOpenPercent', fallback: 15, min: 0, max: 100 },
  { key: 'nightMinOpenPercent', label: 'Ночной минимум (%)', helpKey: 'climate.nightMinOpenPercent', fallback: 15, min: 0, max: 100 },
  { key: 'nightMaxOpenPercent', label: 'Ночной максимум (%)', helpKey: 'climate.nightMaxOpenPercent', fallback: 20, min: 0, max: 100 },
  { key: 'dayBaseOpenPercent', label: 'Дневное базовое (%)', helpKey: 'climate.dayBaseOpenPercent', fallback: 10, min: 0, max: 100 },
  { key: 'dayMinOpenPercent', label: 'Дневной минимум (%)', helpKey: 'climate.dayMinOpenPercent', fallback: 15, min: 0, max: 100 },
  { key: 'dayMaxOpenPercent', label: 'Дневной максимум (%)', helpKey: 'climate.dayMaxOpenPercent', fallback: 85, min: 0, max: 100 },
  { key: 'tempFullOpenDeltaC', label: 'Дельта полного открытия по температуре (°C)', helpKey: 'climate.tempFullOpenDeltaC', fallback: 6, min: 0.1, max: 30, step: 0.1 },
...
const weatherFields: NumericField[] = [
  { key: 'outsideHotterGain', label: 'Коэффициент «снаружи горячее»', helpKey: 'climate.outsideHotterGain', fallback: 1, min: 0, max: 10, step: 0.1 },
  { key: 'outsideWetterGain', label: 'Коэффициент «снаружи влажнее»', helpKey: 'climate.outsideWetterGain', fallback: 1, min: 0, max: 10, step: 0.1 },
  { key: 'windReduceThresholdMs', label: 'Порог снижения ветра (м/с)', helpKey: 'climate.windReduceThresholdMs', fallback: 8, min: 0, max: 100, step: 0.1 },
  { key: 'windCloseThresholdMs', label: 'Порог закрытия ветра (м/с)', helpKey: 'climate.windCloseThresholdMs', fallback: 12, min: 0, max: 100, step: 0.1 },
  { key: 'windReduceWindwardMaxPercent', label: 'Макс. наветренная при ветре (%)', helpKey: 'climate.windReduceWindwardMaxPercent', fallback: 25, min: 0, max: 100 },
  { key: 'windReduceLeewardMaxPercent', label: 'Макс. подветренная при ветре (%)', helpKey: 'climate.windReduceLeewardMaxPercent', fallback: 50, min: 0, max: 100 },
  { key: 'windStormWindwardMaxPercent', label: 'Макс. наветренная при шторме (%)', helpKey: 'climate.windStormWindwardMaxPercent', fallback: 0, min: 0, max: 100 },
  { key: 'windStormLeewardMaxPercent', label: 'Макс. подветренная при шторме (%)', helpKey: 'climate.windStormLeewardMaxPercent', fallback: 10, min: 0, max: 100 },
  { key: 'rainWindwardPositionPercent', label: 'Наветренная при дожде (%)', helpKey: 'climate.rainWindwardPositionPercent', fallback: 0, min: 0, max: 100 },
  { key: 'rainLeewardPositionPercent', label: 'Подветренная при дожде (%)', helpKey: 'climate.rainLeewardPositionPercent', fallback: 10, min: 0, max: 100 },
  { key: 'rainUnknownDirectionMaxPercent', label: 'Макс. при дожде без направления (%)', helpKey: 'climate.rainUnknownDirectionMaxPercent', fallback: 5, min: 0, max: 100 },
  { key: 'overheatEmergencyTempC', label: 'Аварийный перегрев (°C)', helpKey: 'climate.overheatEmergencyTempC', fallback: 38, min: 20, max: 80, step: 0.1 },
]

const orientationFields: NumericField[] = [
  { key: 'greenhouseOrientationDeg', label: 'Ориентация теплицы (°)', helpKey: 'climate.greenhouseOrientationDeg', fallback: 0, min: 0, max: 359.999, step: 0.1 },
  { key: 'leftRoofNormalDeg', label: 'Нормаль левой кровли (°)', helpKey: 'climate.leftRoofNormalDeg', fallback: 0, min: 0, max: 359.999, step: 0.1 },
  { key: 'rightRoofNormalDeg', label: 'Нормаль правой кровли (°)', helpKey: 'climate.rightRoofNormalDeg', fallback: 0, min: 0, max: 359.999, step: 0.1 },
]
