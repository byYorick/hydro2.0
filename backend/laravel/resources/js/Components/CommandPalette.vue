...
const roleBasedCommands = computed<CommandItem[]>(() => {
  ...
  if (isOperator.value || isAdmin.value) {
    commands.push(
      { type: 'nav', label: 'Мастер настройки системы', icon: '⚙️', category: 'Настройка', action: () => safeVisit('/setup/wizard') },
      { type: 'nav', label: 'Создать теплицу', icon: '🏠', category: 'Создание', action: () => safeVisit('/greenhouses/create') },
    )
  }
