<template>
  <Card v-if="hasFavorites" class="mb-4">
    ...
    <div v-if="favoriteZonesCount > 0" class="mb-3">
      <div class="text-xs text-neutral-400 mb-2">Зоны ({{ favoriteZonesCount }})</div>
      <div class="flex flex-wrap gap-2">
        <Link
          v-for="zoneId in favoriteZones"
          :key="zoneId"
          :href="`/zones/${zoneId}`"
          class="px-2 py-1 rounded border text-xs transition-colors hover:border-neutral-600 bg-neutral-900 border-neutral-800"
        >
          {{ getZoneName(zoneId) }}
          <button
            @click.stop="removeZone(zoneId)"
            class="ml-1.5 text-neutral-500 hover:text-neutral-200"
    ...
    <div v-if="favoriteDevicesCount > 0">
      <div class="text-xs text-neutral-400 mb-2">Устройства ({{ favoriteDevicesCount }})</div>
      <div class="flex flex-wrap gap-2">
        <Link
    ...
    <!-- Пустое состояние -->
    <div v-if="!hasFavorites" class="text-xs text-neutral-500 text-center py-4">
