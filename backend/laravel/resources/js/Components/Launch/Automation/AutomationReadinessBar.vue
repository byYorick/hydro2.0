    ...
    <section class="cal-readiness">
        <div class="cal-readiness__header">
            <div class="cal-readiness__title">
                <span class="cal-readiness__label">готовность автоматики</span>
...
defineEmits<{
    ...
    (e: 'refresh'): void;
    (e: 'open-contract', contract: AutomationContract): void;
}>();
...
function statusLabel(s: AutomationContractStatus): string {
    return s === 'passed' ? 'пройден' : s === 'blocker' ? 'блокер' : s === 'active' ? 'активный' : 'опционально';
}
</script>
...
.cal-readiness__segment--passed { background: rgb(34, 197, 94); }
.cal-readiness__segment--blocker { background: rgb(251, 191, 36); }
.cal-readiness__segment--active { background: rgb(56, 189, 248); }
.cal-readiness__segment--optional { background: rgba(148, 163, 184, 0.4); }
</style>
