<template>
  <Card class="flex flex-col gap-4 surface-card-hover transition-all duration-200">
    <div class="flex justify-between gap-4 items-start min-w-0">
      <div class="min-w-0 flex-1">
        ...
        <p
          ...
          class="text-xs text-[color:var(--text-dim)] mt-1 max-h-10 overflow-hidden"
        >
    ...
    <div class="grid grid-cols-2 gap-2 md:grid-cols-4 text-xs text-[color:var(--text-muted)]">
      <div
        v-for="status in zoneStatusList"
        :key="status.key"
        class="surface-strong p-3 rounded-xl border border-[color:var(--border-muted)] min-w-0 overflow-hidden"
      >
    ...
    <div class="grid grid-cols-2 gap-2 text-xs">
      <div class="surface-strong p-3 rounded-xl border border-[color:var(--border-muted)]">
        <div class="text-[color:var(--text-muted)] text-[0.65rem] uppercase tracking-[0.2em] mb-2">
          Узлы
        </div>
      ...
      <div class="surface-strong p-3 rounded-xl border border-[color:var(--border-muted)]">
        ...
        <div class="text-2xl font-bold text-[color:var(--accent-amber)] text-right">
          {{ greenhouse.alerts_count ?? 0 }}
        </div>
        <div class="text-xs text-[color:var(--text-dim)] text-right">
          активных
        </div>
      </div>
    </div>
      ...
      <div class="space-y-1">
        ...
        >
          <div class="min-w-0 flex-1">
            ...
            <div class="text-xs text-[color:var(--text-dim)] truncate">
              {{ zone.description || 'Без описания' }}
            </div>
          </div>
          <Badge
            :variant="zone.status === 'ALARM' ? 'danger' : 'warning'"
            size="xs"
            class="flex-shrink-0"
          >
            {{ zone.status }}
