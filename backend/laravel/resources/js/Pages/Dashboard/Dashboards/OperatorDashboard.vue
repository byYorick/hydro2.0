<template>
  <div class="space-y-6">
    ...
    <div
      v-if="zonesNeedingAttention.length > 0"
      class="space-y-4"
    >
      <div class="flex items-center justify-between">
        ...
        <Button
          size="sm"
          variant="secondary"
          @click="resolveIssues(zonesNeedingAttention[0]?.id)"
        >
          Следующая
        </Button>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Card
          ...
          :key="zone.id"
          class="border-[color:var(--badge-warning-border)] bg-[color:var(--badge-warning-bg)]"
        >
          ...
          <div class="text-xs text-[color:var(--accent-red)] mt-2">
            Алертов: {{ (zone as any).alerts_count ?? 0 }}
          </div>
        </Card>
      </div>
    </div>
