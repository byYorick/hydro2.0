...
export function useAutomationRuntimeMeta(
  ...
  const hasFailed = computed(() => Boolean(automationState.value?.state_details?.failed))

  const errorCode = computed(() => automationState.value?.state_details?.error_code ?? null)

  const errorMessage = computed(() => automationState.value?.state_details?.error_message ?? null)
