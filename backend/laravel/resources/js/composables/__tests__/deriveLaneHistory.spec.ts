...
describe('deriveLaneHistory', () => {
  ...
  it('группирует runs по task_type и расставляет % позиции в горизонте 24h', () => {
    ...
    expect(byLane.irrigation[0].kind).toBe('executed')
    expect(byLane.irrigation[0].execution_id).toBe('a')
    expect(byLane.irrigation[1].t).toBe(50)
    expect(byLane.irrigation[1].s).toBe('run')
    expect(byLane.irrigation[1].kind).toBe('executed')
  ...
  it('добавляет будущие окна в ленту, даже если run ещё не создан', () => {
    ...
    expect(lanes[0].runs[0].kind).toBe('planned')
    expect(lanes[0].runs[0].at).toBe('2026-02-10T16:00:00.000Z')
