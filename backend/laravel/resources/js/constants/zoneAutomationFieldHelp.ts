export const ZONE_AUTOMATION_FIELD_HELP: Record<string, string> = {
  ...
  'water.irrigationRecoveryEnabled': 'Включает post-irrigation recovery, если цели pH/EC не достигнуты к концу полива. Выключено — сразу переход в ready.',
  'water.irrigationRecoveryMaxContinueAttempts': 'Максимум окон коррекции pH/EC в irrigation_recovery до fail-closed (irrigation_recovery_stop_failed).',
  'water.irrigationRecoveryTimeoutSeconds': 'Общий таймаут recovery-фазы после полива.',
  'water.irrigationAutoReplayAfterSetup': 'После setup из-за solution_min автоматически повторить полив (irrigation_start). Также разрешает AE3 replay normal irrigation после setup/recovery, если initial launch был отложен или оборван safety-path. Выключено — задача завершится без replay.',
