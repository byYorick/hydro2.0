...
CREATE TABLE public.ae_tasks (
    ...
    CONSTRAINT ae_tasks_corr_step_check CHECK (((corr_step IS NULL) OR ((corr_step)::text = ANY ((ARRAY['corr_activate'::character varying, 'corr_wait_stable'::character varying, 'corr_check'::character varying, 'corr_dose_ec'::character varying, 'corr_wait_ec'::character varying, 'corr_dose_ph'::character varying, 'corr_wait_ph'::character varying, 'corr_dose_ph_piggyback'::character varying, 'corr_wait_ph_piggyback'::character varying, 'corr_dilute_pulse'::character varying, 'corr_dilute_settle'::character varying, 'corr_deactivate'::character varying, 'corr_done'::character varying])::text[])))),
