...
class ZoneAutomationStartSolutionChangeController extends Controller
{
    ...
    public function store(Request $request, Zone $zone): JsonResponse
    {
        ...
        $payload = [
            ...
            'idempotency_key' => $validated['idempotency_key'] ?? $this->buildIdempotencyKey(zoneId: $zone->id),
        ];

        try {
            return response()->json($this->bridge->dispatchStartSolutionChange($zone->id, $payload));
            ...
            Log::warning('ZoneAutomationStartSolutionChangeController: automation-engine request failed', [
                'zone_id' => $zone->id,
                'error' => $e->getMessage(),
            ]);

            return $this->localizedError('upstream_error', 'Ошибка при запуске подмены раствора в automation-engine.', 503);
        } catch (ConnectionException $e) {
            Log::warning('ZoneAutomationStartSolutionChangeController: automation-engine unavailable', [
                'zone_id' => $zone->id,
                'error' => $e->getMessage(),
            ]);
    ...
    {
        return Str::lower("zone-{$zoneId}-solution-change-".Str::uuid());
    }
}
