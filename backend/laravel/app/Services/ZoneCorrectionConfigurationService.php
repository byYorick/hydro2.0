...
class ZoneCorrectionConfigurationService
{
    ...
    public function getOrCreateForZone(Zone|int $zone): ZoneCorrectionConfiguration
    {
    ...
    public function getResponsePayload(Zone|int $zone): array
    {
        ...
        $pumpOverride = $this->extractPumpCalibrationOverride($config->baseConfig);
        ...
        return [
            ...
            'phase_overrides' => $config->phaseOverrides,
            'resolved_config' => $resolvedConfig,
            'version' => $config->version,
            'updated_at' => optional($config->updatedAt)->toISOString(),
            'updated_by' => $config->updatedBy,
            'last_applied_at' => optional($config->lastAppliedAt)->toISOString(),
            'last_applied_version' => $config->lastAppliedVersion,
            'meta' => [
    ...
    public function upsert(
        ...
        return DB::transaction(function () use ($zoneId, $preset, $baseConfig, $phaseOverrides, $userId): ZoneCorrectionConfiguration {
            $pumpDefaults = $this->documents->getSystemPayloadByLegacyNamespace('pump_calibration', true);
    ...
    private function makeTransientConfig(
        ...
        return new ZoneCorrectionConfiguration(
            id: $documentId,
            zoneId: $zoneId,
            presetId: $presetId,
            preset: $presetId ? $this->findCorrectionPreset($presetId) : null,
            baseConfig: is_array($payload['base_config'] ?? null) ? $payload['base_config'] : [],
            phaseOverrides: is_array($payload['phase_overrides'] ?? null) ? $payload['phase_overrides'] : [],
            resolvedConfig: $this->normalizeResolvedConfigVersion(
            ...
            version: $version,
            updatedBy: $updatedBy,
            updatedAt: $this->coerceDateTime($updatedAt),
            lastAppliedAt: $this->coerceDateTime($payload['last_applied_at'] ?? null),
            lastAppliedVersion: isset($payload['last_applied_version']) ? (int) $payload['last_applied_version'] : null,
        );
    }
     ...
     */
    private function normalizeResolvedConfigVersion(array $resolvedConfig, array $phaseOverrides, int $version): array
    {
