...
configure_php_runtime_overrides() {
    local app_env="${APP_ENV:-production}"
    local override_file="/usr/local/etc/php/conf.d/zz-runtime-opcache.ini"
    local opcache_enable="${PHP_OPCACHE_ENABLE:-}"
    local opcache_enable_cli="${PHP_OPCACHE_ENABLE_CLI:-}"
    local opcache_validate_timestamps="${PHP_OPCACHE_VALIDATE_TIMESTAMPS:-}"
    local opcache_revalidate_freq="${PHP_OPCACHE_REVALIDATE_FREQ:-}"

    if [ -z "$opcache_enable" ] && [ -z "$opcache_enable_cli" ] && [ -z "$opcache_validate_timestamps" ] && [ -z "$opcache_revalidate_freq" ]; then
        if [ "$app_env" = "local" ] || [ "$app_env" = "testing" ] || [ "$app_env" = "e2e" ]; then
            opcache_enable="0"
