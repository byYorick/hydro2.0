...
def test_gating_blocks_when_workflow_phase_not_open():
    ...
    assert state["reason_code"] == "workflow_phase_not_open"
    assert state["stale_flags"] == []


def test_gating_allows_workflow_open_phase_with_fresh_flags():
    now = datetime.now(timezone.utc)
    logger = _Logger()
    state = build_correction_gating_state(
        telemetry={},
...
def test_gating_fail_closed_for_stale_flags_in_open_workflow_phase():
    now = datetime.now(timezone.utc)
    logger = _Logger()
    state = build_correction_gating_state(
        telemetry={},
