...
def _resolve_critical_diff_threshold(correction_type: str) -> float:
    ...
    if correction_key == "ec":
        return float(getattr(settings, "AE_EC_CRITICAL_DIFF_THRESHOLD", 0.30))
    return float(getattr(settings, "AE_PH_CRITICAL_DIFF_THRESHOLD", 0.50))
...
async def should_apply_correction(
    ...
    """Политика решения по корректировке на основе тренда и порогов."""
    ...
    # NOTE: DB cooldown (zone_events) отключён, чтобы не дублировать PID min_interval_ms.
    # Коррекция для некритичных отклонений определяется только по тренду и min_diff.
    ...
    # Минимальное отклонение для коррекции (вне dead_zone) берётся из settings
    settings = get_settings()
    correction_key = str(correction_type or "").strip().lower()
    if correction_key == "ec":
        min_diff = float(getattr(settings, "AE_CORRECTION_MIN_DIFF_EC", 0.10))
    else:
        min_diff = float(getattr(settings, "AE_CORRECTION_MIN_DIFF_PH", 0.15))
    ...
    # Отклонение небольшое (< min_diff), не корректируем
...
async def should_apply_proactive_correction(
    ...
    """Gate для proactive-correction внутри dead-zone без DB cooldown."""
    settings = get_settings()
    correction_key = str(correction_type or "").strip().lower()
    if correction_key == "ec":
        min_diff = float(getattr(settings, "AE_CORRECTION_MIN_DIFF_EC", 0.10))
        ...
        min_diff = float(getattr(settings, "AE_CORRECTION_MIN_DIFF_PH", 0.15))
    if abs(float(projected_diff)) <= min_diff:
