...
async def _validate_irr_state_expected_vs_actual(
    self,
    *,
    zone_id: int,
    workflow: str,
    ...
    expected = _CRITICAL_IRR_STATE_EXPECTATIONS.get(workflow)
    if not expected:
        return None
    ...
    if snapshot_entry is None:
        # Даже при timeout=0 даем небольшой grace-период для eventual consistency по cmd_id.
    ...
    if snapshot_entry is None and cmd_id_filter:
        # Fallback: если snapshot по cmd_id не найден, но есть очень свежий latest snapshot,
        # используем его, чтобы не падать на race между publish и IRR_STATE_SNAPSHOT.
        latest_snapshot = await _load_latest_irr_state_snapshot(self, zone_id=zone_id)
        latest_created_at = (
            latest_snapshot.get("created_at") if isinstance(latest_snapshot, dict) else None
        )
    ...
    if age_sec is not None and age_sec > float(max_age_sec) and not cmd_id_filter:
        # Если latest snapshot оказался stale, даем короткое окно на появление нового snapshot.
...
async def _request_irr_state_snapshot_best_effort(
    self,
    *,
    zone_id: int,
    workflow: str,
...
async def execute_two_tank_startup_workflow_core(
    ...
    *,
    ...
    state_cmd_id = await _request_irr_state_snapshot_best_effort(
        self,
        zone_id=zone_id,
        workflow=workflow,
    )
    irr_state_guard_result = await _validate_irr_state_expected_vs_actual(
        self,
        zone_id=zone_id,
        workflow=workflow,
