...
@pytest.mark.asyncio
async def test_execute_device_task_core_resolves_pump_alert_when_safety_passes(monkeypatch):
    ...
    async def _get_zone_nodes(_zone_id, _node_types):
        return [{"id": 8, "uid": "nd-test-irrig-1", "channel": "pump_main"}]
    ...
    async def _send_alert(**_kwargs):
        raise AssertionError("send_infra_alert should not be called when safety gate passes")

    async def _send_resolved_alert(**kwargs):
        resolved_alerts.append(dict(kwargs))
    ...
    result = await execute_device_task_core(
        ...
        publish_batch_fn=_publish_batch,
        send_infra_alert_fn=_send_alert,
        send_infra_resolved_alert_fn=_send_resolved_alert,
        err_mapping_not_found="mapping_not_found",
        err_no_online_nodes="no_online_nodes",
    )
    ...
    assert resolved_alerts[0]["code"] == "infra_irrigation_pump_blocked"
    assert resolved_alerts[0]["component"] == "scheduler_task_executor"
