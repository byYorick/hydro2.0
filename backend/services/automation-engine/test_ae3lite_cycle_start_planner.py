...
def test_cycle_start_planner_uses_run_pump_duration_for_irrigation_start() -> None:
    now = datetime.now(timezone.utc)
    planner = CycleStartPlanner()
    task = AutomationTask.from_row({
        "id": 73, "zone_id": 9, "task_type": "irrigation_start", "status": "running",
        "idempotency_key": "planner-irrigation-duration", "scheduled_for": now, "due_at": now,
        "claimed_by": "worker-a", "claimed_at": now, "error_code": None, "error_message": None,
        "created_at": now, "updated_at": now, "completed_at": None,
        "topology": "two_tank", "intent_source": None, "intent_trigger": None,
        "intent_id": None, "intent_meta": {},
        "current_stage": "irrigation_start", "workflow_phase": "irrigating",
        "stage_deadline_at": None, "stage_retry_count": 0, "stage_entered_at": None,
        "clean_fill_cycle": 0, "corr_step": None,
        ...
        **{
            **_snapshot().__dict__,
            "targets": {
                "ph": {"target": 5.9},
                "ec": {"target": 1.4},
                "irrigation": {"duration_sec": 300, "interval_sec": 3600},
            },
            "diagnostics_execution": {
                "workflow": "cycle_start",
                "topology": "two_tank_drip_substrate_trays",
    ...
    plan = planner.build(task=task, snapshot=snapshot)
    start_cmds = plan.named_plans["irrigation_start"]
    pump_cmd = next(cmd for cmd in start_cmds if cmd.channel == "pump_main")

    assert pump_cmd.payload["cmd"] == "run_pump"
    assert pump_cmd.payload["params"]["duration_ms"] == 120_000
