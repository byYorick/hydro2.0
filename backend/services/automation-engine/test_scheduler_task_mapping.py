...
def test_default_mapping_for_irrigation():
    ...
    assert mapping.node_types == ("irrig",)
    assert mapping.default_duration_sec == 60.0


def test_mapping_override_normalizes_node_types_to_canonical():
    mapping = get_task_mapping(
        "mist",
        {
            "execution": {
                "cmd": "set_relay",
                "node_types": ["climate", "CLIMATE_NODE", "fogger"],
    ...
    assert mapping.node_types == ("climate",)
