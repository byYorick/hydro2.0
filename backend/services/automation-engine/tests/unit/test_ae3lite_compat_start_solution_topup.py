...
@pytest.mark.asyncio
async def test_compat_start_solution_topup_translates_busy_error_to_409() -> None:
    endpoint, captured = _bind_test_route(
...
@pytest.mark.asyncio
async def test_compat_start_solution_topup_claim_race_maps_to_409_zone_busy() -> None:
    endpoint, captured = _bind_test_route(
        creation_result=TaskCreationResult(task=_task(task_id=779, zone_id=5, status="pending"), created=True),
        decision="claim_race",
    )
