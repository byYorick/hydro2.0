...
Скрипт создаст файл `passwords.txt` с хешами паролей для всех пользователей.
...
mqtt:
  volumes:
    ...
    - ./services/mqtt-bridge/passwords.txt:/mosquitto/config/passwords:ro
...
mosquitto_passwd -c passwords.txt test_user
...
**Решение:**
1. Проверьте файл `passwords.txt`
...
**Причина:** Файлы `passwords.txt` или `acl` не смонтированы в контейнер.
...
3. **Обновите docker-compose:**
   - Добавьте volumes для `passwords.txt` и `acl`
