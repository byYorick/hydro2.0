...
# Запускаем контрактные тесты (новый формат с .schema.json и fixtures/)
echo "1. Тестирование JSON схем для telemetry, command, command_response, alert, zone_events..."
echo "   Используются схемы: *.schema.json"
echo "   Используются fixtures: fixtures/*.json"
pytest common/schemas/test_contracts.py -v --tb=short

if [ $? -ne 0 ]; then
    echo "❌ Контрактные тесты провалились - несовместимый payload обнаружен!"
    echo "   Проверьте fixtures в common/schemas/fixtures/ против схем в common/schemas/*.schema.json"
    exit 1
fi

echo "✅ Контрактные тесты прошли успешно"

# Запускаем старые протокольные тесты (если есть, для обратной совместимости)
if [ -f "common/schemas/test_protocol_contracts.py" ]; then
    echo ""
    echo "2. Запуск протокольных тестов (legacy)..."
    pytest common/schemas/test_protocol_contracts.py -v --tb=short || {
        echo "⚠️  Протокольные тесты провалились (legacy тесты, не критично)"
    }
else
    echo ""
    echo "2. Файл test_protocol_contracts.py не найден, пропускаем"
fi

# Запускаем chaos тесты (если доступны зависимости)
echo ""
echo "3. Запуск chaos тестов..."
...
echo ""
echo "4. Авто-проверка WS событий с event_id..."
