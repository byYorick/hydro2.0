...
| Нет автодолива бака в фазе роста | **Закрыто** (`solution_topup`) | **B** |
| Нет контура/алертов t° раствора | Алерты HL + Prometheus `SolutionTempOutOfBand`; контур нагрева — опционально C.2 | **C** |
| Нет подмены раствора / CIP | **D.1 закрыто** (`solution_change` drain→clean_fill rinse→refill). CIP с санитайзером — D.2 | **D** |
| Внешний алертинг (Telegram) отсутствует | Код есть (`TelegramAlertNotifier`); нужен `TELEGRAM_BOT_TOKEN` + `alerts:telegram-test` | **E** |
| Firmware fail-safe при потере связи | Код + default `link_loss_timeout_sec=30` (NodeConfig + firmware fallback) | **F** |
