...
> Узел не должен публиковать telemetry с полем `ts` до завершения time sync через `hydro/time/response`.
...
- QoS = 1
- Retain = true
- Публикация выполняется только после завершения time sync через `hydro/time/response`
