...
- этого достаточно для happy-path и retry-limit сценариев, но не гарантирует воспроизводимый strict same-window target после полного рестарта всего e2e стека;
...
- после такого seed сценарий всё ещё требует строгого доведения до runtime target window:
  - `pH 5.00..5.15`
  - `EC 2.30..2.38`
- это не ослабление acceptance criteria, а фиксация каноничной стартовой точки для проверки именно same-window semantics (`EC` и `PH` в одном correction window), а не cold-start endurance от `reset_state`.
