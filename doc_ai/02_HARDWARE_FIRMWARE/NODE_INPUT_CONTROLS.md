...
- Этот документ задаёт **целевую** унифицированную модель ввода.
- В текущем runtime production-нод нет общего `input_task` и `ui_input_event_t`.
- Фактически используются:
  - `factory_reset_button` (долгое удержание GPIO-кнопки, full NVS erase + reboot),
  - API OLED UI (`oled_ui_handle_encoder`, `oled_ui_handle_button`) в местах, где есть физический ввод.
...
Примечание: перечисление выше — целевой контракт. В текущей кодовой базе этот typedef не внедрён как общий runtime API.
...
Текущий runtime-статус: отдельная универсальная `input_task` не реализована.
...
Текущий runtime-статус: для production-нод используется `factory_reset_button` c отдельным polling task
и длительным удержанием (по умолчанию 20 секунд).

---

# 6. Требования к ИИ-агенту

1. ИИ не имеет права добавлять прямые обращения к GPIO/энкодеру в UI-слое;
   ввод должен идти через выделенный input-модуль (`input_task` в целевой архитектуре или специализированный runtime-компонент вроде `factory_reset_button`).
