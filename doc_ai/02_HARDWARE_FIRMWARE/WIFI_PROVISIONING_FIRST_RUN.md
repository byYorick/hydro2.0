...
4. Пользователь через Android-приложение или браузер:
 - подключается к AP,
 - вводит SSID/пароль Wi-Fi и параметры MQTT (`mqtt_host`, `mqtt_port`).
...
- тайм-аут автоостановки setup-портала не реализован;
- режим ожидания конфигурации продолжается до получения валидных данных.
...
- `GET /` — простая HTML-страница с формой:
 - поля: `ssid`, `password`, `mqtt_host`, `mqtt_port`.
- `POST /wifi/connect` — приём JSON.
...
Runtime-ограничения текущей реализации `setup_portal`:
- `mqtt_host` валидируется только как IPv4 (`xxx.xxx.xxx.xxx`);
- DNS hostname (например, `broker.local`) через `POST /wifi/connect` сейчас не принимается;
- размер POST payload ограничен ~512 байт.
...
Целевой вариант хранения может использовать отдельный namespace `wifi_config`
с ключами:
...
Текущая реализация хранит это внутри общего NodeConfig в `config_storage`:
- `wifi.ssid`, `wifi.pass`;
- `mqtt.host`, `mqtt.port`;
- `node_id`, `gh_uid`, `zone_uid` (при отсутствии создаются/сохраняются temp/default значения).
...
Текущая реализация `factory_reset_button` использует полный `nvs_flash_erase()` (full wipe NVS).
Это приводит к очистке не только Wi‑Fi, но и остальных NVS-данных.
...
2. **Через прямой HTTP-клиент**:
 - подключается к AP,
 - отправляет `POST /wifi/connect` программно.

Интеграция описывается в `../12_ANDROID_APP/ANDROID_APP_API_INTEGRATION.md`.
