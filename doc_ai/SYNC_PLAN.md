...
- `docs/` — устаревший mirror, не является source of truth

**Действия:**
1. Удалить корневую папку `docs/`.
2. Оставить `doc_ai/` единственным source of truth.
3. Убрать из актуальных гайдов рекомендации mirror-правок в `docs/`.

**Оценка:** 30 минут  
**Статус:** ✅ Выполнено — корневой legacy mirror `docs/` удалён, актуальные ссылки ведут в `doc_ai/`
...
1. Решить судьбу `backend/services/api-gateway/`:
   ...
   - Вариант В: Оставить как placeholder с пометкой в README ✅ (выбрано)
...
3. Обновить `doc_ai/01_SYSTEM/01_PROJECT_STRUCTURE_PROD.md`:
   - Исправить описание api-gateway (указать legacy статус и роль Laravel) ✅

**Оценка:** 45 минут  
**Статус:** ✅ Выполнено — документация обновлена, api-gateway помечен как legacy, роль API Gateway выполняет Laravel
...
**Проблема:** Документ упоминался в `IMPLEMENTATION_STATUS.md`, но отсутствовал

**Понимание:**
- Каноническая спецификация находится в `doc_ai/04_BACKEND_CORE/PYTHON_SERVICES_ARCH.md`
- Локальная техническая entrypoint-копия создана в `backend/services/PYTHON_SERVICES_ARCH.md`

**Действия:**
1. Использовать `doc_ai/04_BACKEND_CORE/PYTHON_SERVICES_ARCH.md` как source of truth.
2. Использовать `backend/services/PYTHON_SERVICES_ARCH.md` как локальный указатель для разработчиков сервисов.
3. Новые изменения архитектуры фиксировать сначала в `doc_ai/04_BACKEND_CORE/PYTHON_SERVICES_ARCH.md`.
...
**Проблема:** Документ упоминался в документации, но отсутствовал

**Понимание:**
- Каноническая спецификация находится в `doc_ai/02_HARDWARE_FIRMWARE/NODE_CONFIG_SPEC.md`
- Локальная копия для разработчиков прошивок создана в `firmware/NODE_CONFIG_SPEC.md`

**Действия:**
1. Использовать `doc_ai/02_HARDWARE_FIRMWARE/NODE_CONFIG_SPEC.md` как source of truth.
2. Использовать `firmware/NODE_CONFIG_SPEC.md` как локальную копию для firmware-разработки.
3. При изменении NodeConfig синхронизировать оба файла.
...
2. Обновить `firmware/nodes/common/components/README.md`:
...
2. **Решения требуются:**
   - Судьба `api-gateway` (удалить или реализовать согласно документации) ✅ (решено: оставлен как legacy placeholder, роль выполняет Laravel)
   - Судьба `device-registry` (удалить или реализовать согласно документации) ✅ (решено: оставлен как legacy placeholder, функционал реализован в Laravel)
