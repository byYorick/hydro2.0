...
### 1.4. Реализованный workflow_stage (scheduler_task_executor.py)
...
### 1.4.1. Canonical naming: `workflow_stage` vs `workflow_phase`

Чтобы не смешивать внутренние шаги планировщика и доменные состояния, в этом документе используется фиксированный нейминг:

- `workflow_stage` — внутренние шаги `SchedulerTaskExecutor`:
  `startup`, `clean_fill_check`, `solution_fill_check`, `prepare_recirculation_check`, `irrigation_recovery`, `irrigation_recovery_check`.
- `workflow_phase` — доменные состояния для координации с ZAS:
  `idle`, `tank_filling`, `tank_recirc`, `ready`, `irrigating`, `irrig_recirc`.

Базовый mapping stage -> phase:

- `startup`/`clean_fill_check`/`solution_fill_check` -> `tank_filling`
- `prepare_recirculation_check` -> `tank_recirc`
- `DONE` (после startup) -> `ready`
- `irrigation_recovery`/`irrigation_recovery_check` -> `irrig_recirc`
- `DONE` (после recovery-ветки) -> `irrigating` (если recovery запускался во время полива) или `ready` (если recovery был в startup-контуре); правило должно быть явным в коде.
...
### 3.1. Разделение задач на 3 ИИ-агента

| Агент | Основной scope | Этапы |
|------|-----------------|-------|
| Agent 1 | Contract + Gating foundations | P0, P1 |
| Agent 2 | Workflow Coordination | P2 |
| Agent 3 | Event Integrity + Persistence + Decomposition | P3, P4, P5 |

Документы по агентам:

- `doc_ai/10_AI_DEV_GUIDES/AUTOMATION_ENGINE_AGENT_1_CONTRACT_GATING.md`
- `doc_ai/10_AI_DEV_GUIDES/AUTOMATION_ENGINE_AGENT_2_WORKFLOW_COORDINATION.md`
- `doc_ai/10_AI_DEV_GUIDES/AUTOMATION_ENGINE_AGENT_3_INTEGRITY_PERSISTENCE_DECOMPOSITION.md`

Порядок исполнения по зависимостям: Agent 1 -> Agent 2 -> Agent 3.
...
2. SchedulerTaskExecutor обновляет `workflow_phase` при переходах `workflow_stage`, используя canonical-набор: `idle`, `tank_filling`, `tank_recirc`, `ready`, `irrigating`, `irrig_recirc`
...
3. При входе в `workflow_stage=solution_fill_check` → установить `workflow_phase = "tank_filling"`; при входе в `workflow_stage=prepare_recirculation_check` → установить `workflow_phase = "tank_recirc"`
4. ZAS в correction gating: если `workflow_phase in {"tank_filling","tank_recirc"}` → разрешить коррекцию (workflow уже запустил насос/рециркуляцию)
5. Убрать из ZAS логику `activate_sensor_mode` при `missing_flags` (ownership только у workflow)

**Тесты:**
- E2E-01: Full startup workflow_stage cycle: startup → clean_fill_check → solution_fill_check → prepare_recirculation_check → targets_reached
- E2E-13: solution_fill_check_start → activate_sensor_mode sent
...
**Проблема в коде:** `pid_state_manager.py` — PID state сохраняется по `(zone_id, pid_type)`, нет понятия «фаза». При смене `tank_recirc` → `irrigating` EC target меняется, но integral заряжен от предыдущей фазы.

**Что сделать:**
1. При переходе `workflow_phase` (`tank_recirc` → `irrigating`) → `del ph_controller._pid_by_zone[zone_id]`
2. Аналогично для ec_controller
3. PID пересоздаётся с новым target при следующей коррекции

**Тесты:**
- E2E-12: `tank_recirc` → `irrigating` → PID integral reset

**Exit criteria для P2:** E2E координации фаз зелёные; нет команд полива вне `workflow_phase in {"ready","irrigating"}`.
...
**Статус (2026-02-16):** частично выполнено.
- `scheduler_task_executor.py` — thin coordinator API (без `exec`-loader).
- Core workflow блоки вынесены из `application/scheduler_executor_impl.py` в:
  - `domain/workflows/two_tank_core.py`
  - `domain/workflows/three_tank_core.py`
  - `domain/workflows/cycle_start_core.py`
- `application/scheduler_executor_impl.py` сокращён (с ~5.8k до ~4.35k строк), но целевой порог `<800` ещё не достигнут.
...
| E2E-10 | NPK-only during tank_filling/tank_recirc | BUG-03 | `test_workflow_coordination.py` | P2 |
...
**Статус (2026-02-16):** структура `tests/e2e/` и файлы E2E-01..E2E-27 добавлены как исполняемая матрица-обёртка поверх существующих сценариев сервиса.
...
**Детальное structured logging (обязательно для всех этапов P0-P5):**
- Единый минимальный набор полей в каждом ключевом логе:
  `component`, `zone_id`, `task_id`, `task_type`, `workflow`, `workflow_phase`,
  `decision`, `reason_code`, `result_status`, `correlation_id`, `duration_ms`.
- Для recovery-path дополнительно:
  `recovery_action`, `state_age_sec`, `workflow_selected`, `fallback_from`, `fallback_to`.
- Для command-dispatch дополнительно:
  `command_count`, `commands_failed`, `terminal_status`, `cmd_id`.
- Запрещены "немые" fallback-ветки: каждая деградация должна логироваться как минимум `WARNING`
  и иметь machine-readable `reason_code`.
