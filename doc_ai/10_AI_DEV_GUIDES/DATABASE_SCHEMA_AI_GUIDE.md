...
3. **Схема документируется.**
 - Изменения должны быть отражены в `DATA_MODEL_REFERENCE.md`.
...
- При добавлении сущностей:
 - описать их в `DATA_MODEL_REFERENCE.md`;
 - при необходимости — уточнить здесь, как AI может их использовать.
- AI-движок (см. `AI_OPTIMIZATION_ENGINE.md`) может требовать дополнительные таблицы
...
- документация (`DATA_MODEL_REFERENCE.md`, этот файл) обновляется;
