...
- `api-gateway` — упоминается как отдельный сервис, но фактически это Laravel
- `device-registry` — упоминается, но не реализован
- **Решение:** Обновить документацию, указав реальное состояние
