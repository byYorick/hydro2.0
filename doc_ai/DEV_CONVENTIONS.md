...
1. **Source of Truth**  
   ...
   - Защищённые контракты (MQTT, команды, схема БД, HL API, AE3 ingress/FSM, retention) — canonical в `doc_ai/`; обязаны совпадать с кодом. При drift правим docs **или** явно чиним код.  
   - Архитектурный обзор — `SYSTEM_ARCH_FULL.md` и `01_SYSTEM`. Слойные спеки: `02_HARDWARE_FIRMWARE`, `03_TRANSPORT_MQTT`, `04_BACKEND_CORE`, `06_DOMAIN_ZONES_RECIPES`; ИИ/двойник — `09_AI_AND_DIGITAL_TWIN`, `10_AI_DEV_GUIDES`.  
   - Правила агентов — корневой `AGENTS.md`.

2. **Doc-first только для защищённых контрактов**  
   - Цепочка «Документ → Интерфейс → Реализация» обязательна **только** при изменении защищённых контрактов из п.1 — не для любой фичи.  
   - Обычные фичи и багфиксы: код + тесты; `doc_ai/` обновлять, если задет контракт (чек-лист в `AGENTS.md`).
...
5. **Среда разработки**  
   - Backend (Laravel), Python-сервисы, БД, e2e и смежные проверки — в Docker; команды запускать внутри контейнеров проекта.  
   - **Исключение:** прошивки ESP32 собираются на хосте в окружении ESP-IDF (`idf.py`, `source …/esp-idf/export.sh`), вне Docker.  
...
6. **Границы совместимости (проект в разработке)**  
   ...
   - В остальных частях допускаются несовместимые изменения без обратной совместимости.

7. **Freeze новых подсистем**  
...
- Корень репозитория содержит:
  ...
  - Каталоги `01_…`–`10_…`, `12_…`, `13_…` (и архив `00_ARCHIVE`), группирующие артефакты по слоям системы.
...
3. **Именование** (канон: `02_HARDWARE_FIRMWARE/ESP32_C_CODING_STANDARDS.md`)
   - Файлы:
     - Заголовочные: `snake_case.h`
     - Исходники: `snake_case.c`
   - Функции и переменные: `snake_case` (функции — с префиксом компонента)
   - Типы (typedef / enum-типы): `snake_case_t` (например, `wifi_state_t`, `ui_state_t`)
...
2. **API‑контракты**
   ...
   - Изменение **защищённых** контрактов (HL API, AE3 ingress/FSM, MQTT, схема БД, команды, retention) — doc-first: сначала `doc_ai/`, затем код, либо одновременный fix drift.
   - Прочие API — код + тесты; спека вместе с кодом, если меняется публичный контракт.

3. **Создание новых сервисов**
   - Новые Python-сервисы, новые AE3 `task_type`, новые authority document types — **запрещены без явного запроса** пользователя.
   - Даже после запроса: нельзя плодить микросервисы без обоснования (архитектура, доменный сценарий, логи/метрики).
...
- Перед началом работы читать корневой `../AGENTS.md` и локальные AGENTS.md в целевых каталогах.
...
3. **Фронтенд**
   - Unit/Component тесты (Vitest + Vue Test Utils) для Vue компонентов.
   - Интеграционные тесты для страниц Inertia.
   - E2E тесты (Playwright) для критических пользовательских сценариев.
   - Подробная стратегия описана в `07_FRONTEND/FRONTEND_TESTING.md`.
...
4. **Автоматизация**
   ...
   - CI/CD конвейер настроен (GitHub Actions):
