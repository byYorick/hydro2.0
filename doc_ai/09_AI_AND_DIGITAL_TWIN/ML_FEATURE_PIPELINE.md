...
### Phase 0 · Подготовка (✅ DONE — 2026-04-23)

Решения по §18 и верификация состояния зафиксированы в **Приложении C**.
Отдельная ветка `feature/ml-feature-pipeline` **не создавалась** — работаем
прямо в `ae3` (C.2). Retention-синхронизация отложена до prod-развертывания
(C.2, §18.1).

### Phase 1 · Расширение агрегатов (✅ DONE — коммит `a1178a93`)

Задача | Факт
---|---
Миграция §5.1 — колонки `value_std`, `value_p10`, `value_p90`, `slope_per_min`, `valid_count`, `agg_version` | ✅ [migrations/2026_04_23_120000_extend_telemetry_aggregates_for_ml.php](../../backend/laravel/database/migrations/2026_04_23_120000_extend_telemetry_aggregates_for_ml.php) — расширены **обе** таблицы `telemetry_agg_1m` и `telemetry_agg_1h`
SQL в `telemetry-aggregator/main.py`: `STDDEV_SAMP`, `PERCENTILE_CONT(0.1/0.9)`, `REGR_SLOPE` | ✅ Вынесены в `_build_agg_1m_query` / `_build_agg_1h_query`
Quality-фильтр | ✅ `FILTER (WHERE quality='GOOD')` **только** на ML-полях (std/p10/p90/slope/valid_count). Классические avg/min/max/median остаются по всем записям — backward compat (отступление от §5.1, обосновано)
Compression policy | ✅ `add_compression_policy('telemetry_agg_1m', INTERVAL '14 days')`
Unit-тесты | ✅ `test_ml_phase1.py` — 9 тестов, все проходят
Инфраструктура тестов | ✅ `make test-agg` (pytest в requirements + локальный `pytest.ini` + правка `Dockerfile.test`-конвенции)
Бонус: baseline-фикс | ✅ `utcnow()` → `utcnow_naive()` при инициализации `last_ts` (столбец без tz в `telemetry_samples.ts`)

**Факт в dev на момент коммита:** 522 строки `agg_version=2` в `telemetry_agg_1m`,
60 строк в `telemetry_agg_1h`, compression_enabled=t.

### Phase 2A · Миграции ML-витрин + skeleton feature-builder (✅ DONE — коммит `a1ebe595`)

Задача | Факт
---|---
Миграции §5.2, §5.4, §5.8 (`zone_features_5m`, `ml_labels`, `ml_data_quality_windows`) | ✅ [migrations/2026_04_23_130000_create_ml_feature_tables.php](../../backend/laravel/database/migrations/2026_04_23_130000_create_ml_feature_tables.php) — все три таблицы, `timestamptz` (не `timestamp without time zone`), `zone_features_5m` hypertable с compression_segmentby='zone_id'
Корректировка C.6 | ✅ `cycle_id` FK + денормализованные `phase_code varchar(32)`, `phase_id bigint` (без FK), `hours_since_phase_start` в дополнение к `hours_since_cycle_start`
Сервис `feature-builder` (skeleton) | ✅ [backend/services/feature-builder/](../../backend/services/feature-builder/) — FastAPI + `/healthz` + `/readyz` + `/metrics` + poll-loop-заглушка, порт 9410
Регистрация в docker-compose | ✅ `backend/docker-compose.dev.yml` — depends_on db+laravel, healthcheck TCP 9410
Инфраструктура тестов | ✅ `make test-fb` — 5 тестов skeleton'а

**Факт в dev:**
- `curl /healthz` → `{"status":"ok","schema_version":1,"horizons":[5,15,60]}`
- `feature_builder_poll_runs_total` растёт каждую минуту
- Реальной сборки фичей **нет** — это Phase 2B/3

### Phase 2B · Логика сборки витрин (⬜ TODO)

Задача | Предполагаемая разбивка
---|---
Основная логика сборки `zone_features_5m` по 5-мин сетке | Синтетический E2E: инжект telemetry → через 90 сек строка в `zone_features_5m`
...
### Phase 3 · dose_response_events + ml_calibration_events (⬜ TODO)

Задача | DoD
---|---
Миграции §5.3 (скорректировано по C.3), §5.5 | Up/down проходят
Сборка `dose_response_events` из `commands WHERE cmd='dose' AND status='DONE'` (НЕ из `command_audit` — см. C.3) | Тест: доза → через 70 мин есть строка с заполненными `ph_at_plus_60m`
Резолв `reagent` из `commands.node_id + channel` | Документированный маппинг в feature-builder
Snapshot'ы реконструируются из `telemetry_samples` в окнах `[dose_ts-60s, dose_ts]` и `[dose_ts+h ± 30s]` | Unit-тест
`is_clean` логика (нет других доз/доливов в окне ±60 мин) | Unit-тесты на 5 сценариев
...
| Дата | Изменение | Автор | Коммит |
|---|---|---|---|
| 2026-04-23 | Addendum C: верификация состояния, решения по §18, перевод `dose_response_events` с `command_audit` на `commands` | claude (ae3) | `a1178a93` |
| 2026-04-23 | **Phase 1 DONE** — расширены `telemetry_agg_1m/1h` (std/p10/p90/slope/valid_count/agg_version), quality-фильтр на ML-полях, compression policy, baseline-фикс `utcnow_naive`, 9 unit-тестов, `make test-agg` | claude (ae3) | `a1178a93` |
| 2026-04-23 | **Phase 2A DONE** — миграции `zone_features_5m` + `ml_labels` + `ml_data_quality_windows`, skeleton `feature-builder` (FastAPI + /healthz + /metrics + poll-заглушка), `make test-fb` | claude (ae3) | `a1ebe595` |

### C.10. Факт vs план — отступления

Задокументированы, чтобы не забыть:

1. **Quality-фильтр только на ML-полях.** План §5.1 просит `FILTER quality='GOOD'` на всех агрегатах. В реальности фильтр применён только к новым ML-полям (`value_std/p10/p90/slope_per_min/valid_count`); классические `value_avg/min/max/median` считаются по всем записям. Причина: минуты со всеми BAD-семплами иначе дали бы `value_avg IS NULL` и сломали UI/графы. `sample_count` всё = count(*); `valid_count` = count FILTER GOOD.

2. **`ts` в старых агрегатах остался `timestamp without time zone`.** Новые таблицы Phase 2 (`zone_features_5m`, `ml_labels`, `ml_data_quality_windows`) — `timestamptz`. Переводить старые — вне ML-скоупа.

3. **Phase 2 разбита на 2A/2B.** В изначальном плане Phase 2 включала и миграции, и логику сборки (≈ 2 недели). Разделили для чистоты коммитов и точек отката:
   - **2A** — миграции + skeleton сервиса (сделано коммитом `a1ebe595`).
   - **2B** — реальная сборка фичей и правила качества (предстоит).

4. **`aggregator_state.last_ts` исторически NULL.** Починка сделана «по симптому» (`utcnow_naive` вместо `utcnow`), первый успешный цикл начнёт заполнять корректно. Глубокой инвестигации «почему NULL», какой-то data-loss сценарий — не проводилось, т.к. prod-данных нет.
