...
**As-is MVP:** `backend/services/digital-twin/` уже имеет modular solvers, auto-calibration → `zone_dt_params`, live sim / replay (см. `DIGITAL_TWIN_ENGINE.md`).

Этот charter описывает **target** поверх MVP — полноценный what-if / RL-среда, способный:
