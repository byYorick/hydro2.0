...
Recurring-операции (полив, коррекция, recirculation) идут автоматически
**после старта соответствующего этапа**. **Все stateful решения уровня цикла**
(advance phase, manual override stages) требуют действия агронома через UI.
Auto-advance фаз НЕ выполняется — UI показывает notification
"Фаза готова к переходу".

Отличия `semi` от `manual` на уровне runtime (см. инвариант в §5.2):
- `semi` принимает Laravel scheduler intents (полив/освещение/диагностика);
  `manual` — нет.
- На `startup` stage `semi` ведёт себя как `manual`: ждёт `pending_manual_step`
  (`clean_fill_start` / `solution_fill_start`) и не маршрутизирует fill-этап
  автоматически по `clean_max`. Поэтому manual-step `start_*` / `stop_*` в `semi`
  **разрешены** (а не игнорируются). `force_solution_fill_start` — только `manual`.
- После старта этапа check-стадии (`*_check`) в `semi` авто-продвигаются по
  датчикам/дедлайнам (в отличие от `manual`, который ждёт `stop_*` / safety-событие).
...
| Manual-step `start_*` / `stop_*` | ✗ (игнор) | ✓ | ✓ |
| Manual-step `force_solution_fill_start` | ✗ | ✗ | ✓ |
...
Контроль ролей — на уровне route middleware
(`role:operator,admin,agronomist,engineer` для `POST /zones/{id}/manual-step`).
В `auto` AE3 отвечает `409 manual_step_forbidden_in_auto_mode` независимо от роли.
Более гранулярные per-role / per-stage ограничения — out of v1 (§10).
