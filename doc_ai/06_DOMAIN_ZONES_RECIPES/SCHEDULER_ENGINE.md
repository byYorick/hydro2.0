# SCHEDULER_ENGINE.md
# Планировщик автоматизации зон (Laravel) — каноника 2.0

**Статус:** планирование и dispatch расписаний выполняет **Laravel** (`automation:dispatch-schedules`); отдельного контейнера **`scheduler`** в Docker-стеке **нет**.

Compatible-With: Protocol 2.0, Backend >=3.0, Python >=3.0, Database >=3.0, Frontend >=3.0.

---

## 1. Назначение

- сопоставить **расписания полива/света/климата** из рецепта и фаз зоны с моментами запуска;
- создать **намерения (intents)** автоматизации в PostgreSQL;
- **разбудить** `automation-engine` HTTP-вызовом к совместимому endpoint (см. §2);
- отследить lifecycle задач в таблицах scheduler-dispatch (например `laravel_scheduler_active_tasks`, `zone_automation_intents`, `greenhouse_automation_intents`).

Исполнение workflow зоны (команды на узлы, ожидание терминальных статусов, коррекции) остаётся в **automation-engine** и **history-logger** — см. `doc_ai/04_BACKEND_CORE/PYTHON_SERVICES_ARCH.md`, `doc_ai/04_BACKEND_CORE/ae3lite.md`, `doc_ai/ARCHITECTURE_FLOWS.md`.

---

## 2. Поток данных (инвариант)

```
Laravel (cron / schedule:work / automation:dispatch-schedules)
  → запись intent + dispatch state в БД
  → POST к automation-engine (endpoint зависит от типа задачи — см. ниже)
  → … → history-logger → MQTT → ESP32
```

**Wake-up endpoint по типу задачи (фактическая реализация Laravel `ScheduleDispatcher`):**

| Тип расписания (`task_type`) | HTTP endpoint в AE | Примечание |
|------------------------------|---------------------|------------|
| `irrigation` | `POST /zones/{id}/start-irrigation` | intent/task `irrigation_start`, опционально `requested_duration_sec` из payload расписания |
| `lighting` | `POST /zones/{id}/start-lighting-tick` | только при `zones.automation_runtime='ae3'`; intent/task `lighting_tick`; payload `desired_state`/`brightness_pct` (см. `SCHEDULER_AE3_NON_IRRIGATION_DISPATCH.md` §7) |
| `solution_topup` | `POST /zones/{id}/start-solution-topup` | AE3 only; intent `SOLUTION_TOPUP_TICK`, task `solution_topup` (см. `WATER_FLOW_ENGINE.md` §19). **Не** `start-cycle`. |
| `solution_change` | `POST /zones/{id}/start-solution-change` | AE3 only; intent `SOLUTION_CHANGE_TICK`, task `solution_change` (semi-auto; см. `CORRECTION_CYCLE_SPEC.md` §10). **Не** маппить в `cycle_start` / `start-cycle`. |
| greenhouse climate (крыша) | `POST /greenhouses/{id}/start-climate-tick` | после tick `automation:dispatch-schedules` вызывает `GreenhouseClimateDispatchService` → `greenhouse_automation_intents` + `greenhouse_automation_state.next_scheduled_tick_at`; не использовать `start-cycle` зоны (см. `GREENHOUSE_CLIMATE_CONTROL_PLAN.md`) |
| `diagnostics` | `POST /zones/{id}/start-cycle` | на AE3 идёт через compat-path `diagnostics / cycle_start`; intent `DIAGNOSTICS_TICK`, task `cycle_start` |
| прочие зональные (`climate`, `mist`, `ventilation`, …) | — | на зонах с **`automation_runtime='ae3'`** планировщик эти типы **не диспатчит** (`ScheduleDispatcher::isSchedulerTaskTypeDispatchableForAe3`; остаются в `non_executable_planned_task_types`) |

Колонка `zones.automation_runtime` сохранена; в БД CHECK допускает только `'ae3'`. Отдельной матрицы endpoint-ов для non-ae3 runtime нет.

- Публикация команд в MQTT **только** через history-logger.
- HTTP-транспорт `POST /scheduler/task` и `GET /scheduler/task/{task_id}` **удалён** из runtime.

---

## 3. Операционные опоры

- Конфигурация и флаги cutover: переменные `AUTOMATION_LARAVEL_SCHEDULER_*`, синхронизация токенов с `automation-engine` (см. `doc_ai/08_SECURITY_AND_OPS/RUNBOOKS.md`, раздел про планировщик).
- **Tick расписания:** в `routes/console.php` частота `automation:dispatch-schedules` строится из `AUTOMATION_LARAVEL_SCHEDULER_DISPATCH_INTERVAL_SEC` (10–60 с — sub-minute через `everyTenSeconds`…`everyMinute`; >60 с — cron `*/N * * * *`). Внешний cron/`schedule:run` по-прежнему должен срабатывать **каждую минуту** (стандарт Laravel); sub-minute шаги выполняются внутри минутного окна.
- **Lock TTL:** эффективное значение `max(AUTOMATION_LARAVEL_SCHEDULER_LOCK_TTL_SEC, p99(duration scheduler cycle start_cycle) + AUTOMATION_LARAVEL_SCHEDULER_LOCK_TTL_MARGIN_SEC, 10с)` — см. `AutomationRuntimeConfigService::schedulerConfig()` и Cache-lock внутри `automation:dispatch-schedules`. Mutex `Schedule::withoutOverlapping()` на bootstrap использует только `ceil(AUTOMATION_LARAVEL_SCHEDULER_LOCK_TTL_SEC / 60)` минут (без чтения aggregates), чтобы не дергать БД при загрузке `routes/console.php`; параллельные процессы дополнительно отсекаются тем же Cache-lock.
- **Параллелизм HTTP:** `AUTOMATION_LARAVEL_SCHEDULER_DISPATCH_PARALLELISM` — размер пула `Http::pool` на один tick (см. `ScheduleDispatcher::dispatchBatch`).
- Метрики dispatch: публичный endpoint Laravel `GET /api/system/scheduler/metrics` (см. `doc_ai/04_BACKEND_CORE/REST_API_REFERENCE.md`): в т.ч. `laravel_scheduler_pending_intents_count`, `laravel_scheduler_oldest_pending_intent_age_seconds`, `laravel_scheduler_dispatch_cycle_overrun_seconds` (наблюдаемость лага относительно `dispatch_interval_sec`).
- Ответы `409 *_zone_busy` (`start_cycle_zone_busy`, `start_irrigation_zone_busy`, `start_lighting_tick_zone_busy`, `start_solution_topup_zone_busy`, `start_solution_change_zone_busy`) при активной AE3-задаче — ожидаемый `backpressure` single-writer: Laravel **не** пишет `recordSchedulerDispatchFailure`, **не** терминалит intent, курсор окна (lighting OFF) не двигается (`holdCursorOnRetryable`). AE3 **не** вызывает `mark_terminal` на requested intent при `decision=zone_busy` и при retryable `failed` с тем же ключом до проверки busy.
- Для `task_type=irrigation|solution_topup|solution_change` на AE3 scheduler-dispatch обязан проверять `zone_workflow_state.workflow_phase` и при значении, отличном от `ready` (или при отсутствии row), **не** вызывать wake-up: логировать `reason=zone_setup_pending` как retryable backpressure.
- Защитный ingress-контракт AE3 зеркалит это правило: `409 start_irrigation_setup_pending` / `start_solution_topup_not_ready` (и эквивалент change) — backpressure без `recordSchedulerDispatchFailure`; task/lease не создаются.
- UI оператора: schedule workspace и timeline строятся из канонического состояния автоматизации, а не из удалённого Python task API.
- **Watchdog AE3 (`ae3:reap-stale-tasks`, каждую минуту):** **не** пишет `ae_tasks.status=failed` и **не** трогает `zone_workflow_state` (terminal FSM и fail-safe OFF — только ae3lite janitor `stale_task_reconcile`). Laravel логирует stale `claimed|running|waiting_command` (deadline / claim / progress) для hang-hints. Reap только orphan `zone_automation_intents` в `pending` **без** строки `ae_task`. Не терминалит retryable `laravel_scheduler` intents (`retry_count < max_retries`) и lighting/irrigation/solution_* backpressure (lighting OFF / `*_zone_busy`) — skip + лог.
- **Retryable dispatch failure:** при `connection_error` / `http_error` / отсутствии `task_id` в ответе AE3 Laravel инкрементирует `retry_count` intent (`intent_source=laravel_scheduler`); terminal `failed` intent — после `max_retries` (default 3). **Не** применяется к `409 *_zone_busy` и `409 start_irrigation_setup_pending` / `start_solution_topup_not_ready` (backpressure). Битый lighting `desired_state` — fail-closed **skip** (не retry, не default `on`).
- Ответ `GET /api/zones/{id}/schedule-workspace` содержит `capabilities.ae3_irrigation_only_dispatch` (историческое имя: «ограниченный набор типов под автодиспатч на AE3»), `capabilities.executable_task_types` и `capabilities.non_executable_planned_task_types` — источник истины для подсказок оператору на AE3 (см. `doc_ai/04_BACKEND_CORE/API_SPEC_FRONTEND_BACKEND_FULL.md` §3.5.1). На AE3 автодиспатч расписания покрывает **`irrigation`, `lighting`, `solution_topup`, `solution_change`, `diagnostics`** (зеркало `ScheduleDispatcher`); остальные запланированные типы — non-executable.

### 3.1. Ручные расписания зоны (`zone_manual_schedules`)

Операторские правила, дополняющие расписания из рецепта. CRUD: `GET/POST/PUT/DELETE /api/zones/{zone}/manual-schedules` (роли `agronomist`, `admin`). В workspace отдаются в `manual_schedules` и попадают в `plan.windows` с `origin=manual`.

| Поле | Тип | Описание |
|------|-----|----------|
| `task_type` | string | `irrigation`, `lighting`, `diagnostics`, `ventilation`, `mist`, `solution_topup`, `solution_change` |
| `schedule_kind` | string | `time` (HH:MM UTC), `interval` (≥60 с), `window` (start–end UTC), `once` (разово) |
| `days_of_week` | jsonb | ISO 1=Пн … 7=Вс; `null`/`[]` = каждый день (для time/interval/window) |
| `run_at` | timestamptz | Момент для `once` |
| `last_dispatched_at` | timestamptz | Отметка успешного dispatch (`once` авто-выключается) |
| `payload` | jsonb | Напр. `duration_sec` для полива |

Время и дни недели — **UTC** (консистентно с Laravel scheduler-dispatch).

---

## 4. Правила для ИИ-агентов

1. Не описывать и не предлагать отдельный контейнер/процесс планировщика **вне Laravel** как владельца dispatch.
2. Любые изменения в расписании — через Laravel-команды, модели и политики, согласованные с `DATA_MODEL_REFERENCE.md`.
3. Не восстанавливать удалённые endpoint-ы задач планировщика в `automation-engine`.

---

**См. также:** `doc_ai/04_BACKEND_CORE/PYTHON_SERVICES_ARCH.md`, `doc_ai/06_DOMAIN_ZONES_RECIPES/ZONE_LOGIC_FLOW.md`, `doc_ai/07_FRONTEND/FRONTEND_ARCH_FULL.md` (вкладка планировщика зоны).
