...
|---------|---------|------|
| Локальных типов в компонентах | 210 | < 15 |
| Использование `any` | 217 | < 20 |
| Прямые `fetch()` вне `useApi` | 5+ файлов | 0 |
| Неиспользуемые компоненты | 2 | 0 |
| Composables > 600 строк | 3 | 0 |
| Компоненты > 600 строк | 12 | 0 |
| Подтверждённых утечек памяти | 3 | 0 |
| Race condition | 1 | 0 |
| Логических багов | 1 | 0 |

**Frontend:** `backend/laravel/resources/js/`

---

## ФАЗА 0 — Исправление багов (делать первой, не ждёт рефакторинга)
...
## ФАЗА 1 — Критичные архитектурные исправления
...
## ФАЗА 2 — Декомпозиция больших файлов
...
### R2-4 Декомпозиция больших страниц (> 600 строк)

Приоритет по убыванию:

| Файл | Строк | Цель декомпозиции |
|------|-------|-------------------|
| [Pages/Recipes/Edit.vue](../../backend/laravel/resources/js/Pages/Recipes/Edit.vue) | 1097 | Выделить `RecipePhaseEditor`, `RecipeTargetForm`, `RecipeSchedulePanel` |
| [Pages/Alerts/Index.vue](../../backend/laravel/resources/js/Pages/Alerts/Index.vue) | 1031 | Выделить `AlertsFilter`, `AlertsTable`, `AlertDetailPanel` |
| [Pages/Setup/Wizard.vue](../../backend/laravel/resources/js/Pages/Setup/Wizard.vue) | 963 | Выделить отдельные `Step*` компоненты |
| [Pages/Dashboard/Dashboards/AgronomistDashboard.vue](../../backend/laravel/resources/js/Pages/Dashboard/Dashboards/AgronomistDashboard.vue) | 686 | Выделить виджеты в отдельные компоненты |
| [Pages/Devices/Show.vue](../../backend/laravel/resources/js/Pages/Devices/Show.vue) | 680 | Выделить `DeviceChannelsPanel`, `DeviceConfigPanel` |

**Правило:** Страница-контейнер ≤ 200 строк, содержит только оркестрацию и layout. Логика — в composable. Секции — в подкомпонентах.

---

### R2-5 Декомпозиция больших модальных окон

| Компонент | Строк | Действие |
|-----------|-------|---------|
| [Components/PumpCalibrationModal.vue](../../backend/laravel/resources/js/Components/PumpCalibrationModal.vue) | 736 | Вынести логику в `usePumpCalibration.ts` |
| [Components/PlantCreateModal.vue](../../backend/laravel/resources/js/Components/PlantCreateModal.vue) | 723 | Уже есть `usePlantCreateModal.ts`, убрать дублирование состояния |
| [Components/GrowthCycleModal.vue](../../backend/laravel/resources/js/Components/GrowthCycleModal.vue) | 673 | Вынести в `useGrowthCycleModal.ts` |
| [Components/ZoneSimulationModal.vue](../../backend/laravel/resources/js/Components/ZoneSimulationModal.vue) | 672 | Локальные интерфейсы → `types/Simulation.ts` |

---

## ФАЗА 3 — Типизация и архитектурная чистота
...
### R3-4 Убрать `as any` из API-обработчиков
...
СРЕДНИЙ (улучшают поддерживаемость):
  ...
  R2-5   Вынести логику из больших модалей

НИЗКИЙ (техдолг, плановый рефакторинг):
  R2-4   Декомпозиция больших страниц (Recipes/Edit, Alerts/Index, Setup/Wizard)
  R3-1   Вынести 210 локальных типов в types/
  R3-2   Консолидация типов Setup Wizard
  R3-3   Унификация управления модалями
  R3-4   Устранение as any в API-обработчиках
```
