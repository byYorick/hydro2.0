...
- ✅ Обработка `config_response` в `history-logger` для перевода ноды в `ASSIGNED_TO_ZONE`
- ✅ Переход в `ASSIGNED_TO_ZONE` происходит только после получения успешного `config_response` от ноды
