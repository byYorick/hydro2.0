...
2. **Ясное разделение слоёв**: `doc_ai / firmware / backend / mobile / infra`.
...
```text
hydro2.0/
├─ doc_ai/               # Документация и спецификации (единственный source of truth)
...
**Примечание:** Основная документация находится в `doc_ai/`. Корневая папка
`docs/` удалена; новые спецификации и правки документации выполняются только в
`doc_ai/`.
