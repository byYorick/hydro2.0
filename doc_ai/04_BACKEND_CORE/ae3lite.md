...
11. `ae3lite/*` не импортирует legacy runtime packages.
...
5. runtime state irrigation decision/replay хранится в explicit typed columns, а не в legacy `payload`
...
2. не возвращать legacy runtime mode в БД
...
16. В `ae3lite/*` нет импортов из legacy runtime packages.
...
2. runtime API parity с historical legacy runtime
