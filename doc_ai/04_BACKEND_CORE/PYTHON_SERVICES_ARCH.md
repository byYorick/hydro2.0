...
- запуск workflow зоны через `POST /zones/{id}/start-cycle` (cycle/diagnostics) и отдельно штатный полив по расписанию Laravel через `POST /zones/{id}/start-irrigation`;
...
2. Laravel вызывает соответствующий endpoint (для полива — `start-irrigation`, иначе при поддерживаемом runtime — `start-cycle`).
