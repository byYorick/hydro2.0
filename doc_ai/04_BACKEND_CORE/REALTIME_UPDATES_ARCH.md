...
Рекомендуем каноничную схему:

- **WebSocket-соединение** от фронтенда/Android к backend:

 - один endpoint, например `/ws/realtime`;
 - авторизация тем же токеном, что и REST.

Альтернативно (на будущее):

- MQTT-bridge для фронта, если будет использоваться MQTT-клиент в UI.
 В текущей версии 2.0 считаем **WebSocket основным каналом**.
