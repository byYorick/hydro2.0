# AE3FULL: Канонический план реализации Automation Engine v3 (ARCHIVED)

**Версия:** 2.0-canonical (historical)  
**Дата:** 2026-03-06  
...
## 21. Фазовый план реализации (доработан под текущий baseline AE на 2026-03-06)

### 21.0 Текущий baseline (as-is, подтверждён по коду)

1. Scheduler ingress и poller-совместимость уже живут на `start-cycle` + `intent-<id>`:
   `backend/laravel/app/Services/AutomationScheduler/{SchedulerConstants.php,ActiveTaskPoller.php}`.
2. AE runtime остаётся AE2-Lite (`backend/services/automation-engine/ae2lite/*`), отдельного пакета `ae3/` пока нет.
3. Legacy task mapping уже централизован в
   `backend/services/automation-engine/config/scheduler_task_mapping.py`.
4. Single-writer baseline уже реализован в
   `backend/services/automation-engine/ae2lite/api_runtime_concurrency.py`.
5. Command terminal vocabulary и политика `DONE/NO_EFFECT/...` уже закреплены в runtime-трекере
   `backend/services/automation-engine/infrastructure/command_tracker_runtime.py`.
6. PID persisted fields `last_output_ms`, `last_dose_at`, `prev_derivative`, `current_zone`
   уже используются в `backend/services/automation-engine/services/pid_state_manager.py`.

Вывод: план должен идти не "с нуля", а как controlled extraction из AE2-Lite в новый `ae3/` runtime с dual-run и жёсткими rollback gates.

### Фаза 0: Baseline Lock + Contract Freeze (2-3 дня) — Лид: Agent-1

1. Зафиксировать freeze-артефакты из §28 отдельным RFC-коммитом.
2. Формально закрепить baseline-файлы из `21.0` как "не переписывать без feature-flag".
3. Подготовить миграции-каркас для таблиц §11 (без включения runtime).
4. Утвердить ownership matrix и handoff-контракты между агентами.

Gate/DoD:
1. RFC freeze merged.
2. CI green.
3. Ни один production path поведения AE2-Lite не изменён.

### Фаза 1: AE3 Skeleton + Domain Core (4-6 дней) — Лид: Agent-1

1. Создать каркас `backend/services/automation-engine/ae3/` по структуре §23.
2. Реализовать domain-слой: entities, value objects, policies, events, version adapters.
3. Добавить unit tests domain-first (без I/O).
4. Подготовить DDL migration PR (без cutover включения).

Gate/DoD:
1. Domain tests green, coverage domain >= 90%.
2. `ae3/` собирается и изолирован от AE2 runtime путей.
3. Миграции применяются/откатываются в dev без ручного DDL.

### Фаза 2: Application Layer + Queue Semantics (4-6 дней) — Лид: Agent-2

1. Реализовать use cases из §8 поверх ports (без внешнего side-effect по умолчанию).
2. Ввести claim/lease/fence flow и `FinalizeTaskUseCase` с atomic finalize (§10.9).
3. Реализовать recovery stale `waiting_command` (§10.4).
4. Включить contract tests для task lifecycle/status mapping.

Gate/DoD:
1. `pending -> ... -> terminal` проходит в integration с test DB.
2. Duplicate enqueue дедуплицируется через `idempotency_key`.
3. Нет двух активных writer на одну зону в concurrency-тесте.

### Фаза 3: Infrastructure Adapters + Shadow Outbox (5-7 дней) — Лид: Agent-3

1. Реализовать PostgreSQL repositories, LISTEN/NOTIFY adapter (§10.7), outbox publisher (§10.13).
2. Подключить history-logger adapter только через `POST /commands`.
3. Реализовать actuator lock CAS (§10.5) и метрики §25.
4. Запустить shadow-mode: AE3 считает и пишет своё состояние, но не владеет production routing.

Gate/DoD:
1. Integration/fault tests зелёные (lost notify, deadlock, tracker timeout, worker crash).
2. `ae_outbox_backlog` и `ae_notify_lag_seconds` наблюдаемы.
3. Shadow-mode не влияет на поведение AE2-Lite.

### Фаза 4: Legacy Bridge + Dual-Run Canary (5-8 дней) — Лид: Agent-4

1. Включить dual-run phases A/B/C из §14.3 с interlock.
2. Сохранить внешний контракт `start-cycle` + poller vocabulary.
3. Ввести canary routing policy (§31): seed -> 5% -> 25%.
4. Прогнать E2E цепочку:
   `scheduler -> start-cycle -> intent alias -> ae3 task -> history-logger -> terminal -> poller completed`.

Gate/DoD:
1. Backward compatibility подтверждена e2e.
2. Rollback thresholds §25.4 валидированы на canary.
3. Ни одного interlock breach без автоматического safe-stop.

### Фаза 5: Full Cutover + Legacy Decommission Prep (4-6 дней) — Лид: Agent-4

1. Canary rollout до 100% при соблюдении SLO.
2. Переключить source of truth статусов на `ae_tasks` (phase D).
3. Подготовить deprecate plan phase E для bridge кода (без мгновенного удаления).
4. Провести tabletop runbook-сценарии §26 и оформить post-cutover report.

Gate/DoD:
1. SLO из §25 выдержаны на 100% зон не менее 24 часов.
2. Автоматический rollback проверен.
3. PR на удаление legacy path открыт отдельно и защищён feature-flag rollback.

---

## 22. Plan для 4 AI-агентов (execution ownership, обновлённый)

| Агент | Слой | Обязательные deliverables по фазам |
|---|---|---|
| **Agent-1** | Domain + Contracts | RFC freeze, `ae3/domain/*`, status/id/schema contracts, migrations draft, version adapters |
| **Agent-2** | Application Orchestration | `ae3/application/*`, use cases §8, lease/fence/atomic finalize, lifecycle contract tests |
| **Agent-3** | Infrastructure Reliability | `ae3/infrastructure/*`, repositories, notify/outbox/tracker, actuator locks, fault-injection readiness |
| **Agent-4** | Bridge + Ops + Validation | Laravel bridge compatibility, canary rollout, rollback automation, e2e/load/runbook verification |

Правила handoff:
1. Каждый агент завершает фазу только с артефактами: код + тесты + обновлённые метрики/алерты + changelog.
2. Запрещён cross-layer рефакторинг без RFC и согласования с владельцем слоя.
3. Любое изменение freeze-артефакта из §28 делается только через RFC-коммит.
