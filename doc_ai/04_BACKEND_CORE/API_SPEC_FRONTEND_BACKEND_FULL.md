...
- `GET /api/system/scheduler/metrics` - Prometheus exposition для Laravel scheduler
...
- Раздел 8 (System): `GET /api/system/health` и `GET /api/system/scheduler/metrics` публичные, `GET /api/system/config/full` защищен `verify.python.service`
...
Актуальный frontend-contract для zone profile:

- общий greenhouse climate больше не хранится в zonal profile;
- в payload остаются `ph`, `ec`, `irrigation`, `lighting`, `diagnostics`, `solution_change`;
- допускается optional `zone_climate`:
  - `subsystems.zone_climate.enabled: bool`
  - `subsystems.zone_climate.execution: {}`
...
### 3.5.5. GET /api/greenhouses/{id}/automation-logic-profile

- **Аутентификация:** Требуется `auth:sanctum`
- **Описание:** Возвращает сохранённые greenhouse automation profiles и greenhouse climate bindings.
- **Поддерживаемые subsystems:** только `climate`.
- **Ответ (`data`):**
  - `active_mode: \"setup\"|\"working\"|null`
  - `profiles.setup|profiles.working`
  - `bindings.climate_sensors: number[]`
  - `bindings.weather_station_sensors: number[]`
  - `bindings.vent_actuators: number[]`
  - `bindings.fan_actuators: number[]`

### 3.5.6. POST /api/greenhouses/{id}/automation-logic-profile

- **Аутентификация:** Требуется `auth:sanctum`, роль `operator` или `admin` или `agronomist` или `engineer`
- **Описание:** Upsert greenhouse automation profile.
- **Тело:**
  - `mode: \"setup\"|\"working\"`
  - `activate: bool` (optional, default=true)
  - `subsystems.climate.enabled: bool`
  - `subsystems.climate.execution: object`
- **Ограничение v1:** runtime-dispatch для greenhouse climate пока не выполняется; endpoint сохраняет источник истины для UI и будущего runtime.
...
- **Ключевые поля ответа:**
  - `status: ok`
  - `data.control_mode`: `auto | semi | manual`
...
- **Ограничение:** при отсутствии active canonical task endpoint возвращает `409 manual_step_no_active_task`.
...
- Серверная валидация unified шага `4. Автоматизация и устройства зоны` в мастере настройки.
- Проверяет:
...
- **Аутентификация:** Требуется `auth:sanctum`, роль `operator`, `admin`, `agronomist`, `engineer`
- Применение unified шага `4. Автоматизация и устройства зоны`:
...
### 3.9.9. POST /api/setup-wizard/validate-greenhouse-climate-devices
...
- Правила:
  - при `enabled=true` нужен минимум один `climate_sensor`;
  - при `enabled=true` нужен минимум один actuator из `vent_actuator|fan_actuator`;
  - `weather_station_sensor` опционален.

### 3.9.10. POST /api/setup-wizard/apply-greenhouse-climate-bindings
...
- **Аутентификация:** Требуется `auth:sanctum`, роль `operator` или `admin` или `agronomist` или `engineer`
- Атомарное создание plant + recipe + draft revision + phases + publish в одной транзакции.
...
- `GET /api/zones/{zone}/correction-config`
  - возвращает `base_config`, `phase_overrides`, `resolved_config`
  - `resolved_config.pump_calibration` обязателен для AE3-Lite runtime
  - `meta` включает `pump_calibration_defaults` и `pump_calibration_field_catalog`
- `PUT|PATCH /api/zones/{zone}/correction-config`
  - принимает zone-level override в `base_config.pump_calibration`
  - сохраняет только diff относительно `system_automation_settings(namespace='pump_calibration')`
...
- `PUT /api/zones/{zone}/pump-calibrations/{channelId}`
  ...
  - validation min/max берётся из `system_automation_settings(namespace='pump_calibration')`
- `GET /api/zones/{zone}/sensor-calibrations/status`
  - агрегированный статус sensor channels зоны:
    `calibration_status = ok|warning|critical|never`
  - использует thresholds из `system_automation_settings(namespace='sensor_calibration')`
- `POST /api/zones/{zone}/sensor-calibrations`
  - body: `{ "node_channel_id": integer, "sensor_type": "ph"|"ec" }`
  - создаёт calibration session со статусом `started`
...
### 8.3. GET /api/system/scheduler/metrics

- **Аутентификация:** Публичный эндпоинт
- **Формат ответа:** `text/plain; version=0.0.4`
- **Назначение:** Prometheus scrape endpoint для Laravel scheduler.
- **Источник данных:** персистентные aggregate tables; `scheduler_logs` остаются diagnostics-only и не участвуют в реконструкции `counter`/`histogram`.
