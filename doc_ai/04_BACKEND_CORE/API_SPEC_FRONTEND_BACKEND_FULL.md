...
authority-документ `zone.logic_profile` через API `/api/automation-configs/zone/{zone}/zone.logic_profile`
...
- **Аутентификация:** Требуется `auth:sanctum`
- **Описание:** Возвращает authority-документ `zone.logic_profile` для зоны.
- **Ответ (`data`):**
  - `namespace = "zone.logic_profile"`
  - `scope_type = "zone"`
  - `scope_id = {zone_id}`
  - `payload.active_mode: "setup"|"working"|null`
  - `payload.profiles.setup|payload.profiles.working`:
    - `mode`
    - `is_active`
    - `subsystems`
    - `updated_at`
  - `bindings` и UI-метаданные могут возвращаться отдельно в response envelope

### 3.5.4. PUT /api/automation-configs/zone/{id}/zone.logic_profile

- **Аутентификация:** Требуется `auth:sanctum`, роль `operator` или `admin` или `agronomist` или `engineer`
- **Описание:** Полностью сохраняет authority-документ `zone.logic_profile`.
- **Тело:**
  - `payload.active_mode: "setup"|"working"|null`
  - `payload.profiles: object`
- **Валидация:** runtime-конфиг подсистем сохраняется только внутри `payload.profiles.*.subsystems`.
...
### 3.5.5. GET /api/automation-configs/greenhouse/{id}/greenhouse.logic_profile
...
- `GET /api/zones/{zone}/sensor-calibrations/status`
  ...
  - использует thresholds из `system.sensor_calibration_policy`
