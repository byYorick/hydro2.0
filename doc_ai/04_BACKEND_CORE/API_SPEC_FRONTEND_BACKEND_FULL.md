...
- **Допустимые `taskId`:** legacy `st-*`, legacy `intent-*`, а также canonical numeric AE3 task id.
...
### 3.5.6. POST /api/zones/{id}/automation/manual-resume (REMOVED)
...
### 3.5.7. GET /api/zones/{id}/control-mode
...
- **Аутентификация:** Требуется `auth:sanctum`, роль `operator`
- **Описание:** Запуск ручного шага 2-бакового workflow в активном `ZoneRunner`.
...
- **Ограничение:** при активной задаче зоны (`accepted|running`) endpoint возвращает `409 manual_step_zone_busy`
  с `active_task_id` и `active_task_status`.
- **Ограничение:** в `control_mode=semi` ручной шаг разрешен только в активной workflow-фазе
  (`tank_filling|tank_recirc|irrigating|irrig_recirc`), иначе `409 manual_step_zone_inactive`.
- **Поведение:** в `control_mode=manual` и `workflow_phase=idle` ручные шаги разрешены;
  `allowed_manual_steps` содержит полный список manual-step.
- **Manual-only policy:** при `control_mode=manual` automation-engine отключает автоматические действия
  (climate/irrigation/recirculation/pH/EC и scheduler auto tasks) и оставляет только явные `manual-step`.
- **Decision contract:** любые не-`manual_step` scheduler-задачи в `control_mode=manual` получают `no_action`
  с `reason_code=manual_mode_only` (без публикации auto-команд).
- **Ограничение:** список `allowed_manual_steps` phase-aware; шаг вне списка для текущей фазы
  возвращает `409 manual_step_not_allowed_for_phase`.
- **Тело запроса:**
```json
{
