...
**Версия:** 1.2
**Дата:** 2026-07-08
**Статус:** Детализирующий контракт для AE3 / history-logger (+ consumer `solution_topup` в `ready`)
...
### 7.5 `solution_topup` consumer в `workflow_phase=ready`

> Детальная доменная семантика: `../06_DOMAIN_ZONES_RECIPES/WATER_FLOW_ENGINE.md` §19.

В фазе роста (`workflow_phase='ready'`) AE3 становится **потребителем**
`level_switch_changed` для контура автодолива `solution_topup`.

#### 7.5.1. Когда event будит topup-path

После `worker.kick()` AE3 выполняет reconcile и **может** инициировать
`POST /zones/{id}/start-solution-topup` (internal) / create task `solution_topup`,
если read-model подтверждает **need topup**:

- `level_solution_min` triggered (`state=true` в telemetry/events);
- `level_solution_max` not triggered (`state=false`);
- `solution_topup_enabled=true`;
- cooldown expired;
- нет active conflicting task.

Релевантные fast-path edges:

| Канал | Edge | Действие AE3 |
|-------|------|--------------|
| `level_solution_max` | `1 → 0` | Wake-up → reconcile → optional topup start |
| `level_solution_max` | `0 → 1` | Wake-up → active topup may complete (check stage) |
| `level_solution_min` | `1 → 0` | **Не topup** — `GuardSolutionTankStartupResetUseCase` (ready → startup) |
| `level_solution_min` | `0 → 1` | Observability only; не auto-start без `max=false` |

#### 7.5.2. DB-first правила (как для startup stages)

`level_switch_changed` **не может** быть единственным основанием для:

- открытия клапанов/насоса topup;
- terminal `SOLUTION_TOPUP_DONE`;
- skip fail-safe guards (`leak`, `source empty`, timeout).

Канонический порядок:

1. event → `worker.kick()`;
2. read-model: `telemetry_last`, `zone_events`, `zone_workflow_state`, active task;
3. handler `solution_topup_check` принимает решение (как `solution_fill_check`).

Coarse level для UI/dispatch gating:

```text
nutrient_tank_level_percent = 50  → need topup (min on, max off)
nutrient_tank_level_percent = 100 → hold (max on)
nutrient_tank_level_percent = 0   → depleted guard, not topup
```

Источник: `level_monitor.coarse_solution_tank_level_percent`.

#### 7.5.3. Сосуществование с startup-reset guard

При `level_solution_min` depletion (`state=false`) в `ready`:

- **приоритет:** `GuardSolutionTankStartupResetUseCase` (workflow → idle/startup);
- **topup запрещён** — нельзя «долить» из пустого бака без startup workflow.

Guard и topup оба подписаны на `LEVEL_SWITCH_CHANGED`, но имеют disjoint outcomes:

| Read-model | Consumer | Outcome |
|------------|----------|---------|
| min=0 | startup-reset guard | reset to startup |
| min=1, max=0 | solution_topup | start/maintain fill to max |
| min=1, max=1 | neither | hold |

#### 7.5.4. Active topup: interpretation during fill

Пока task `solution_topup` в stage `solution_topup_check`:

- `level_solution_max` `0 → 1` — success hint → reconcile → stop plan (mirror
  `solution_fill_completed` semantics);
- `level_solution_min` `1 → 0` после guard delay — leak hint → fail-closed stop
  (mirror `solution_fill_leak_detected`);
- `level_clean_min` `1 → 0` после guard delay — source empty hint;
- aggregate `storage_state/event` коды `solution_topup_*` (**planned**) — fast-path
  shortcut, не замена DB confirmation.

#### 7.5.5. Idempotency

- `initial=true` на `level_solution_max` при `ready` + `max=true` → no topup;
- duplicate `level_solution_max` `1→0` during cooldown → no new task;
- at-least-once delivery не должен создавать параллельные topup tasks
  (arbitration через intent idempotency + single active task).
...
- aggregate `SOLUTION_TOPUP_DONE` / `SOLUTION_TOPUP_TIMEOUT` / `SOLUTION_TOPUP_*` (planned, §7.5)
- task `solution_topup` stages в AE3 execution UI
...
- в `ready` падение `level_solution_max` без `solution_min` loss → индикатор «нужен topup»;
...
7. В `workflow_phase=ready` AE3 корректно различает depletion guard (`solution_min` lost) и topup candidate (`min` on, `max` off) по read-model, а не только по одному event payload.
...
- `../06_DOMAIN_ZONES_RECIPES/WATER_FLOW_ENGINE.md` (§19 `solution_topup`)
- `AGRO_AUTONOMY_MASTER_PLAN.md` (этап B)
