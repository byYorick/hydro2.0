...
**Общая оценка:** Кодовая база зрелая и хорошо структурированная. После сверки документа с актуальным кодом часть исходных замечаний переквалифицирована: подтверждены несколько практических проблем, часть пунктов оказалась устаревшей, а часть относится скорее к hardening/observability, чем к production bug. На 2026-03-14 дополнительно закрыты операционные замечания по bounded timeout/readiness в AE worker и по Prometheus metrics persistence на стороне Laravel scheduler.
...
### ARCH-11: Scheduler Prometheus metrics больше не зависят от retention `scheduler_logs`

**Статус:** закрыто 2026-03-14

Историческое замечание из отдельного аудита про неверную реконструкцию `counter`/`histogram` из `scheduler_logs` больше не актуально.

Текущее состояние:
- Laravel scheduler пишет персистентные агрегаты в отдельные таблицы:
  - `laravel_scheduler_dispatch_metric_totals`
  - `laravel_scheduler_cycle_duration_aggregates`
  - `laravel_scheduler_cycle_duration_bucket_counts`
- `/api/system/scheduler/metrics` читает именно эти aggregate tables;
- `scheduler_logs` остаются diagnostics-only и не влияют на монотонность Prometheus-метрик после `logs:cleanup`.

Следствие:
- `laravel_scheduler_dispatches_total` теперь устойчив к retention cleanup;
- histogram `laravel_scheduler_cycle_duration_seconds` больше не «сбрасывается» при удалении старых log rows;
- оставшийся открытый вопрос относится уже не к корректности метрик, а к модели доступа к public endpoint.
