...
| PR8 | 🟢 Done | MetricWindowValidator, correction dose/clamp/multi_parallel, alert-block cap, structured logs |
...
**Статус 2026-07-09:** **done** — D4, D5, D6, D7, D8, D11, observe-window bounds, `max_dose_ms` default 60s,
control_mode/E-STOP gates, structured logs (zone_id/task_id/correction_window_id), docs.

### Инструкции

1. **`MetricWindowValidator`** — модуль `ae3lite/domain/services/metric_window_validator.py`
   (sanity bounds pH∈[0,14], EC∈[0,20]). Подключить в decision-window **и** observe-window
   (`correction.py` finalize) → reason `sensor_out_of_bounds`. *(observe-path: done 2026-07-09)*
2. **Clamp дозы** — при срезании duration до `max_dose_ms` пересчитывать `effective_ml`;
   события `EC_DOSING`/`PH_CORRECTED` содержат `effective_ml` + `requested_ml`.
   Default AE3 `max_dose_ms=60_000` (firmware-aligned). *(done 2026-07-09)*
3. **`last_dose_at` после DONE** — писать только при terminal `DONE` дозирующей команды
   (`_resolve_dose_completed_at`). *(done 2026-07-09)*
4. **Fail-closed конфиг**: отсутствие `solution_volume_l` → `PlannerConfigurationError`
   (поле остаётся required metadata, не входит в PID ml math — см. CORRECTION_CYCLE_SPEC).
   `far_zone <= close_zone` → `PlannerConfigurationError` на этапе build плана *(D11 — done)*.
5. **Cap на retry alert-block** (`AE_CORRECTION_ALERT_BLOCK_MAX_RETRIES`, default 10) → terminal fail
   `correction_blocked_by_no_effect_alert` *(D7 — done)*.
6. **`multi_parallel`**: в командах каждого компонента передавать согласованную пару `ml` + `duration_ms`
   после clamp *(D5 — done)*.
7. **Логи**: warning'и stale/clamped/discarded/out-of-bounds — с `task_id`, `correction_window_id`, `zone_id` *(done)*.
8. Обновить `CORRECTION_CYCLE_SPEC.md` / `EFFECTIVE_TARGETS_SPEC.md` при затрагивании targets
   *(частично done 2026-07-09)*.

### Тесты

- DecisionWindowReader / observe: pH=-1/EC=999 → `sensor_out_of_bounds`, не PID/no-effect update.
- Clamp: dose при cap 60_000 мс → команда с `effective_ml`, событие содержит оба значения.
- Fail дозы → `last_dose_at` не записан.
