...
**Статус 2026-07-09:** частично **done** — D4, D6, observe-window bounds, `max_dose_ms` default 60s,
control_mode/E-STOP gates, docs. Остаётся: D5 multi_parallel sync, D7 alert-block cap (если ещё нет),
D11 `far_zone > close_zone`, донастройка логов O6.
...
4. **Fail-closed конфиг**: отсутствие `solution_volume_l` → `PlannerConfigurationError`
   ...
   `far_zone <= close_zone` → `PlannerConfigurationError` на этапе build плана *(D11 — проверить)*.
5. **Cap на retry alert-block** (`AE_CORRECTION_ALERT_BLOCK_MAX_RETRIES`, default 10) → terminal fail
   `correction_blocked_by_no_effect_alert` *(D7 — проверить наличие в коде)*.
6. **`multi_parallel`**: в командах каждого компонента передавать согласованную пару `ml` + `duration_ms`
   после clamp *(D5 — open)*.
7. **Логи**: warning'и stale/clamped — с `task_id`, `correction_window_id`, `zone_id`.
