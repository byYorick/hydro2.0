...
- если runtime видит `prepare_recirculation` и `level_solution_min=0` при включённом guard,
  AE3 обязан остановить recirculation (`prepare_recirculation_solution_low_stop`), снять
  no-effect correction block и перейти в `startup` для обычной подготовки раствора
  (не terminal fail);
...
- `startup_recovery_correction_interrupted` при dose-step (`corr_dose_*`, `corr_wait_*`)
  **не** эскалирует critical сразу: ставит `AE_CORRECTION_INTERRUPT_PENDING_VERIFY`
  + warning `biz_ae3_correction_interrupt_pending_verify`, ждёт online узлов и свежий
  `IRR_STATE_SNAPSHOT`. При OFF-снимке stage → `AE_CORRECTION_INTERRUPT_HARDWARE_SAFE`
  и (для `irrigation_start`) optional auto-replay intent
  (`AE_CORRECTION_INTERRUPT_REPLAY_IRRIGATION`, default off). Critical
  `FLOW_STOP_FAILED_HARDWARE_MAY_BE_ACTIVE` — только если после grace актуаторы ON
  или состояние не подтверждено.
- **v1 heuristic fallback:** если dose-step, но deferred verify недоступен — fail-closed
  эвристика по `corr_step` (см. `emit_correction_interrupted_hardware_risk`).
  Отдельный probe дозирующего EC/pH pump через telemetry — planned.
