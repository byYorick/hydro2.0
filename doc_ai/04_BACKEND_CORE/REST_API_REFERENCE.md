...
| POST | /api/nodes/{id}/ota/trigger | Триггер OTA-обновления узла |
| GET | /api/nodes/{id}/logs | Логи/события по узлу |
...
| GET | /api/channels/{id}/telemetry/history | История по отдельному каналу |
...
| PATCH | /api/alerts/{id}/ack | Подтвердить/принять алерт |
...
| POST | /api/system/cache/refresh | Обновить кэши |
| POST | /api/ai/recipe/suggest | Запрос к AI по предложению рецепта |
| POST | /api/ai/diagnostics/run | Запуск AI-диагностики системы |

---

## 9. Правила расширения REST API для ИИ-агентов
