...
8. [Шаг 7: NodeConfig (read-only)](#8-шаг-7-nodeconfig-read-only)
...
- `uid` - обязательное, максимум 64 символа; при коллизии backend автоматически добавляет суффикс `-2`, `-3`, ...
- `name` - обязательное, максимум 255 символов
- `timezone` - опциональное, максимум 128 символов
- `type` - опциональное, максимум 64 символа
- `greenhouse_type_id` - опциональное; пустые значения `null`, `""`, `"null"` нормализуются в `null`
- `coordinates` - опциональное, массив
- `description` - опциональное, текст
- После создания новая теплица автоматически добавляется в `user_greenhouses` для текущих non-admin пользователей, чтобы strict ACL не ломал следующий шаг мастера.
