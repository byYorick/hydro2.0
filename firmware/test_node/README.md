...
- Команда `calibrate` на `ph_sensor`/`ec_sensor` поддерживается для HIL/e2e sensor calibration:
  terminal `DONE`, обновление in-memory calibration points и публикация `config_report`
...
- Для 2-бакового контура добавлена staged-модель датчиков уровня:
  ...
  - при `clean_fill`:
    - `level_clean_min` может оставаться `0` в начале наполнения (как на production-ноде);
      AE3 **не** применяет clean_fill min-guard / не ждёт `clean_fill_source_empty`
      (поле `clean_fill_min_check_delay_ms` deprecated); пустой источник — AE3 timeout/retry;
    - `level_clean_max=1` через 30 секунд и фиксируется → `clean_fill_completed`;
  - при `solution_fill`:
    ...
    - после delay непрерывно мониторятся `level_clean_min` / `level_solution_min`
      (как production continuous guards).
  ...
  - guard-события (как production): `clean_fill_completed`, `solution_fill_source_empty`,
    `solution_fill_leak_detected`, `recirculation_solution_low`, `irrigation_solution_low`,
    `solution_fill_completed`, `emergency_stop_activated`
    публикуются в `hydro/{gh}/{zone}/nd-test-irrig-1/storage_state/event`
    c `event_code`, `ts`, `snapshot`, `state`.
    `clean_fill_source_empty` — legacy/compat; test-node / production не публикуют.
- Для контуров pH/EC добавлен управляемый тестовый профиль:
  ...
  - базовая амплитуда correction:
    ...
    - EC per-pump (канон: `pump_a=npk`, `pump_b=calcium`, `pump_c=magnesium`, `pump_d=micro`):
      - `pump_a`: `base_delta_EC = 0.068`
      - `pump_b`: `base_delta_EC = 0.055`
      - `pump_c`: `base_delta_EC = 0.035`
      - `pump_d`: `base_delta_EC = 0.018`
      - итог: `delta_EC = base_delta_EC * scale * phase_factor`
    - sequential doses накапливают `pending_ec_delta` (wave/decay не сбрасывается между дозами);
  - dilute physics (EC overshoot / clean-water fill):
    - путь: `pump_main` + `valve_clean_supply` + `valve_solution_fill`;
    - на каждом telemetry tick `ec_value` постепенно снижается к `water_ec_baseline`
      (default `0.45`, обновляется при seed через `set_fault_mode.ec_value`);
  - phase factor:
  ...
  - реакция на сенсоры для correction-каналов масштабируется только по `params.ml`,
    ...
    и отражается в `command_response.details`
    (`channel`, `component`, `ph_after/ec_after`, `delta_*`, `base_delta_ec` для EC,
    `effect_delay_sec`, `effect_decay_sec`).
...
- Добавлена сервисная команда `system/reset_binding` (**sim-only**, не production-канон):
  ...
  - Канонический unbind на real-node / backend: NodeConfig temp namespace (см. `NODE_DETACH_IMPLEMENTATION.md`).
