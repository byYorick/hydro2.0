...
- При получении `.../{node}/config` тест-нода сохраняет namespace в NVS (а полный config — для базовой `nd-test-irrig-1`)
  и сразу публикует `config_report` из текущего состояния.
