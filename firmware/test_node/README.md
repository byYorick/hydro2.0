...
- Команда `state` обрабатывается через отдельный fast-path (выделенная очередь + worker).
  Если fast-path недоступен (например, не удалось создать worker), используется fallback:
...
- Для контуров pH/EC добавлен управляемый тестовый профиль:
  ...
  - при `dose/run_pump` на `pump_acid/pump_base/pump_a..pump_d` реакция на сенсоры масштабируется
    по `params.ml`/`duration_ms` и отражается в `command_response.details` (`ph_after/ec_after`, `delta_*`).
