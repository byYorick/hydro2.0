...
- Для 2-бакового контура добавлена staged-модель датчиков уровня:
  ...
  - при `clean_fill`:
    - `level_clean_min=1` через 10 секунд;
    - `level_clean_max=1` через 30 секунд и фиксируется;
  - при `solution_fill`:
    - `level_solution_min=1` через 10 секунд;
    - `level_solution_max=1` через 60 секунд и фиксируется.
- Для контуров pH/EC добавлен управляемый тестовый профиль:
  ...
  - фактический drift на telemetry tick:
    - `pH += drift + 0.0005`
    - `EC += drift*2 - 0.0025`
  - базовая реакция correction:
    - `pump_acid/pump_base`: `delta_pH = 0.10 * scale * phase_factor`
    - `pump_a..pump_d`: `delta_EC = 0.055 * scale * phase_factor`
  - phase factor:
    - `solution_fill`: `0.50`
    - `tank_recirc`: `pH=2.50`, `EC=3.20`
    - вне этих путей: `1.0`
  ...
  - при `dose/run_pump` на `pump_acid/pump_base/pump_a..pump_d` реакция на сенсоры масштабируется
    по `params.ml`/`duration_ms`, nominal `pH=8 ml`, `EC=12 ml`, clamp `scale=0.5..5.0`,
    и отражается в `command_response.details` (`ph_after/ec_after`, `delta_*`).
...
- `ph_value` — принудительно установить текущее значение `ph_sensor` в диапазоне `4.8..7.2`.
...
- При изменении `*_override`, `ph_value` или `ec_value` тест-нода сразу публикует свежий telemetry snapshot, без ожидания следующего `5s` tick.
...
- `E106_ae3_two_tank_realhw_piggyback_ec_ph_cycle` дополнительно seed-ит recirculation раствор через
  `storage_state/set_fault_mode` в `ph_value=5.68`, `ec_value=1.92` перед strict piggyback-check.
- Это нужно не для послабления target, а чтобы piggyback-сценарий после полного рестарта стенда
  стабильно проверял именно порядок `EC -> PH` и финальное попадание в strict target window,
