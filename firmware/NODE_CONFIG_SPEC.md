...
- Определяет тип узла; каналы (сенсоры/актуаторы) задаются в прошивке и публикуются нодой после привязки
...
| `channels` | array | Да | Массив каналов. ПРИМЕЧАНИЕ: сервер отправляет пустой массив; реальные каналы публикует нода после привязки. |
...
Поле описывает каналы, которые нода отправляет на сервер после привязки. При публикации конфига с сервера на ноду массив пустой, чтобы не переопределять прошивку.
