...
static esp_err_t ph_node_patch_pump_config(void) {
    ...
    if (channels == NULL || !cJSON_IsArray(channels) || cJSON_GetArraySize(channels) == 0) {
        ...
        cJSON *built_channels = ph_node_build_config_channels();
        if (!built_channels) {
            cJSON_Delete(config);
...
esp_err_t ph_node_init_step_oled(ph_node_init_context_t *ctx,
    ...
    char node_id[64];
    get_config_string("node_id", node_id, sizeof(node_id), PH_NODE_DEFAULT_NODE_ID);
...
esp_err_t ph_node_init_step_mqtt(ph_node_init_context_t *ctx,
    ...
    get_config_string("node_id", node_id, sizeof(node_id), PH_NODE_DEFAULT_NODE_ID);
    get_config_string("gh_uid", gh_uid, sizeof(gh_uid), PH_NODE_DEFAULT_GH_UID);
    get_config_string("zone_uid", zone_uid, sizeof(zone_uid), PH_NODE_DEFAULT_ZONE_UID);
