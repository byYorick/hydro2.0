/**
 * @file storage_irrigation_node_app.h
 * @brief Main application header for storage_irrigation_node
 * 
 * Тонкий слой координации - геттеры/сеттеры делегируют в компоненты
 * Объединяет заголовки: tasks, init, telemetry
 */

#ifndef STORAGE_IRRIGATION_NODE_APP_H
#define STORAGE_IRRIGATION_NODE_APP_H

#include "esp_err.h"
#include <stdbool.h>

#ifdef __cplusplus
extern "C" {
#endif

/**
 * @brief Initialize storage_irrigation_node application
 */
void storage_irrigation_node_app_init(void);

/**
 * @brief Start FreeRTOS tasks
 */
void storage_irrigation_node_start_tasks(void);

/**
 * @brief Initialize all storage_irrigation_node components
 * @return ESP_OK on success
 */
esp_err_t storage_irrigation_node_init_components(void);

/**
 * @brief Publish STATUS message
 */
void storage_irrigation_node_publish_status(void);

// State getters - делегируют в компоненты
bool storage_irrigation_node_is_pump_control_initialized(void);

// Node ID getter/setter - использует config_storage
const char* storage_irrigation_node_get_node_id(void);
void storage_irrigation_node_set_node_id(const char *node_id);

#ifdef __cplusplus
}
#endif

#endif // STORAGE_IRRIGATION_NODE_APP_H

