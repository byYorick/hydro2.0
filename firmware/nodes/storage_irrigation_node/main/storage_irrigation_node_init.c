/**
 * @file storage_irrigation_node_init.c
...
void storage_irrigation_node_run_setup_mode(void) {
    ...
    setup_portal_full_config_t config = {
        ...
        .ap_password = STORAGE_IRRIGATION_NODE_SETUP_AP_PASSWORD,
        .enable_oled = true,
        .oled_user_ctx = NULL
    };
...
static void update_oled_connections(void) {
    ...
    // Глушим "I2C error" на storage_irrigation_node и показываем базовый статус
 ...
 */
static void storage_irrigation_node_publish_hello(void) {
    static const char *capabilities[] = {"irrigation", "storage", "pump", "current"};
    esp_err_t err = node_utils_publish_node_hello("irrig", capabilities, 4);
    if (err != ESP_OK) {
...
void storage_irrigation_node_mqtt_connection_cb(bool connected, void *user_ctx) {
    if (connected) {
        ...
        ESP_LOGW(TAG, "MQTT disconnected - storage_irrigation_node is offline");
    }
...
esp_err_t storage_irrigation_node_init_components(void) {
    ESP_LOGI(TAG, "Initializing storage_irrigation_node components...");
    
    storage_irrigation_node_init_context_t init_ctx = {
        .show_oled_steps = true,  // storage_irrigation_node теперь использует OLED
        .user_ctx = NULL
    };
    
    storage_irrigation_node_init_step_result_t step_result = {0};
    
    // [Step 1/6] Config Storage
    esp_err_t err = storage_irrigation_node_init_step_config_storage(&init_ctx, &step_result);
    if (err != ESP_OK) {
    ...
    err = storage_irrigation_node_init_step_wifi(&init_ctx, &step_result);
    if (err == ESP_ERR_NOT_FOUND) {
        // WiFi не настроен - запускаем setup mode
        ESP_LOGW(TAG, "WiFi config not found, starting setup mode...");
        storage_irrigation_node_run_setup_mode();
    ...
    wifi_manager_register_connection_cb(storage_irrigation_node_wifi_connection_cb, NULL);
    ...
    // [Step 3/7] I2C Bus
    err = storage_irrigation_node_init_step_i2c(&init_ctx, &step_result);
    if (err != ESP_OK) {
    ...
    err = storage_irrigation_node_init_step_pumps(&init_ctx, &step_result);
    if (err == ESP_ERR_NOT_FOUND) {
    ...
    // [Step 5/7] OLED UI
    err = storage_irrigation_node_init_step_oled(&init_ctx, &step_result);
    if (err != ESP_OK) {
    ...
    err = storage_irrigation_node_init_step_mqtt(&init_ctx, &step_result);
    if (err != ESP_OK) {
    ...
    esp_err_t fw_err = storage_irrigation_node_framework_init_integration();
    if (fw_err != ESP_OK) {
    ...
    storage_irrigation_node_framework_register_mqtt_handlers();
    ESP_LOGI(TAG, "Using node_framework handlers");
    
    mqtt_manager_register_connection_cb(storage_irrigation_node_mqtt_connection_cb, NULL);
    
    // [Step 7/7] Finalize
    err = storage_irrigation_node_init_step_finalize(&init_ctx, &step_result);
    if (err != ESP_OK) {
