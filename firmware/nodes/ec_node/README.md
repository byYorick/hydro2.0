...
| Шина | SDA | SCL | Устройства |
|------|-----|-----|------------|
| **I2C 0** | GPIO 21 | GPIO 22 | OLED (0x3C), INA209 тока насосов (по конфигу, типично 0x40) |
| **I2C 1** | GPIO 18 | GPIO 19 | Trema EC Flash-I2C (заводской **0x09**, см. iarduino) |

Скорость: 100 kHz (`ec_node_defaults.h`).

Если шина 1 не поднимается (занятые пины, нет разводки), прошивка **остаётся на шине 0** для Trema EC (общая шина с OLED, как раньше) и пишет предупреждение в лог.

Если шина 1 поднята, но датчик **физически на 21/22**, `trema_ec_init()` после неудачи на шине 1 **автоматически ищет модуль на шине 0** и переключает драйвер (в логе: пробуем шину 0).

## Используемый сенсор

Нода использует **Trema EC-сенсор (iarduino)** через I²C:
- Компонент: `trema_ec`
- I²C адрес: **0x09** (заводской; поддерживается **0x08** при discovery)
