...
Драйвер `trema_ec` использует **`I2C_BUS_1`** (`trema_ec.c`: `TREMA_EC_I2C_BUS`). Пины — `ec_node_defaults.h` (`EC_NODE_I2C_BUS_*`).

- Компонент: `trema_ec`
- I²C: первая попытка **`TREMA_EC_ADDR` = 0x09** (iarduino); при отсутствии ответа — скан **0x08–0x77** на **шине 1**; **задокументированный рабочий адрес на стенде — 0x08** (см. `doc_ai/02_HARDWARE_FIRMWARE/NODE_CHANNELS_REFERENCE.md`, раздел 2.2)
