...
static const char *TAG = "ec_node_init";

// Setup mode function
void ec_node_run_setup_mode(void) {
    ESP_LOGI(TAG, "Starting setup mode for EC node");
    ...
    setup_portal_full_config_t config = {
        .node_type_prefix = "EC",
        .ap_password = EC_NODE_SETUP_AP_PASSWORD,
        .enable_oled = true,
        .oled_user_ctx = NULL
    };
...
esp_err_t ec_node_init_components(void) {
    ESP_LOGI(TAG, "Initializing ec_node components...");
    
    ec_node_init_context_t init_ctx = {
        .show_oled_steps = true,
        .user_ctx = NULL
    };
    
    ec_node_init_step_result_t step_result = {0};
    ...
    wifi_manager_register_connection_cb(ec_node_wifi_connection_cb, NULL);
    ...
    // [Step 5/8] OLED UI
    err = ec_node_init_step_oled(&init_ctx, &step_result);
    if (err != ESP_OK) {
    ...
    err = ec_node_init_step_pumps(&init_ctx, &step_result);
    if (err == ESP_ERR_NOT_FOUND) {
    ...
    mqtt_manager_register_connection_cb(ec_node_mqtt_connection_cb, NULL);
