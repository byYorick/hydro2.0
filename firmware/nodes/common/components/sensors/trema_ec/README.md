...
На **ec_node** по умолчанию **`I2C_BUS_1`** (SDA 18 / SCL 19), как Trema pH на **ph_node**; шина 0 — OLED/INA209. При legacy-разводке или сбое init bus 1 драйвер переключается на **`I2C_BUS_0`** (`trema_ec_set_i2c_bus`, fallback в `trema_ec_init`). См. `ec_node_defaults.h`, `trema_ec_get_i2c_bus()`.
