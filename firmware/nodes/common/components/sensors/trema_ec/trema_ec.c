/**
 * @file trema_ec.c
 * @brief Драйвер Trema EC (iarduino_I2C_TDS) — I²C на шине ec_node (см. `trema_ec_get_i2c_bus()`)
 *
...
/** Шина Trema EC: по умолчанию I2C_BUS_1; ec_node_init может переключить на I2C_BUS_0 (legacy wiring). */
static i2c_bus_id_t s_i2c_bus = I2C_BUS_1;
...
static bool trema_ec_addr_valid(uint8_t addr_7bit)
{
    return addr_7bit >= 0x08U && addr_7bit <= 0x77U;
}
...
void trema_ec_set_i2c_address(uint8_t addr_7bit)
{
    ...
    trema_ec_mutex_take();
    s_ec_i2c_addr = addr_7bit;
    trema_ec_mutex_give();
    ESP_LOGI(TAG, "EC I2C 7-bit address set to 0x%02X", addr_7bit);
}
...
static bool trema_ec_read_reg(uint8_t addr, uint8_t reg, uint8_t *buf, size_t len, uint32_t timeout_ms)
{
    for (int attempt = 0; attempt < TREMA_EC_I2C_XFER_MAX_TRIES; attempt++) {
        esp_err_t err = i2c_bus_read_bus(s_i2c_bus, addr, &reg, 1, buf, len, timeout_ms);
        if (err == ESP_OK) {
...
{
    for (int attempt = 0; attempt < TREMA_EC_I2C_XFER_MAX_TRIES; attempt++) {
        esp_err_t err = i2c_bus_write_bus(s_i2c_bus, addr, &reg, 1, buf, len, timeout_ms);
        if (err == ESP_OK) {
...
static bool trema_ec_module_reset(void)
{
    ...
    (void)i2c_bus_reset_bus(s_i2c_bus);
    for (int n = 0; n < 40; n++) {
        uint8_t flg = 0;
...
/**
 * Поиск и привязка Trema EC на текущей `s_i2c_bus`. Вызывать при удержанном mutex.
 * @return true если sensor_initialized выставлен
 */
static bool trema_ec_discover_on_active_bus(void)
{
    s_ec_i2c_addr = TREMA_EC_ADDR;
    uint8_t model = 0;
    uint8_t id4[4];
    ...
    if (!found) {
        if (model == TREMA_EC_DEF_MODEL_TDS) {
            ...
            ESP_LOGW(TAG, "Trema EC: нет валидного ответа на 0x%02X, перебор 0x%02X–0x%02X (bus %d)",
                     s_ec_i2c_addr, TREMA_EC_ADDR_PROBE_FIRST, TREMA_EC_ADDR_PROBE_LAST, (int)s_i2c_bus);
        }
        ...
        for (int ai = TREMA_EC_ADDR_PROBE_FIRST; ai <= TREMA_EC_ADDR_PROBE_LAST; ai++) {
            ...
            s_ec_i2c_addr = addr;
            if (prev != addr) {
                (void)i2c_bus_forget_device(s_i2c_bus, prev);
            }
    ...
    s_init_backoff_until_tick = 0;
    ESP_LOGI(TAG, "Trema EC OK: bus %d, addr 0x%02X", (int)s_i2c_bus, s_ec_i2c_addr);
    return true;
}
...
static bool trema_ec_init_locked(void)
{
    ...
    if (!i2c_bus_is_initialized_bus(s_i2c_bus)) {
        ESP_LOGE(TAG, "I²C bus %d не инициализирован (Trema EC, см. ec_node_defaults.h / trema_ec_set_i2c_bus)",
                 (int)s_i2c_bus);
        last_error = TREMA_EC_ERROR_I2C;
        return false;
    }

    if (trema_ec_discover_on_active_bus()) {
        return true;
    }

    if (s_i2c_bus == I2C_BUS_1 && i2c_bus_is_initialized_bus(I2C_BUS_0)) {
        ESP_LOGI(TAG,
...
static uint8_t trema_ec_get_calibration_locked(uint8_t *info_out)
{
    ...
    if (!sensor_initialized || !i2c_bus_is_initialized_bus(s_i2c_bus)) {
        return 0;
    }
...
bool trema_ec_change_address(uint8_t new_addr_7bit)
{
    ...
    if (old_addr != new_addr) {
        (void)i2c_bus_forget_device(s_i2c_bus, old_addr);
    }
    (void)i2c_bus_forget_device(s_i2c_bus, new_addr);
    last_error = TREMA_EC_ERROR_NONE;
