...
void light_node_run_setup_mode(void) {
    ESP_LOGI(TAG, "Starting setup mode for LIGHT node");
    ...
    setup_portal_full_config_t config = {
        .node_type_prefix = "LIGHT",
        .ap_password = LIGHT_NODE_SETUP_AP_PASSWORD,
        .enable_oled = true,
        .oled_user_ctx = NULL
    };
...
esp_err_t light_node_init_components(void) {
    ESP_LOGI(TAG, "Initializing light_node components...");
    
    light_node_init_context_t init_ctx = {
        .show_oled_steps = true,
        .user_ctx = NULL
    };
    
    light_node_init_step_result_t step_result = {0};
    ...
    err = light_node_init_step_wifi(&init_ctx, &step_result);
    if (err == ESP_ERR_NOT_FOUND) {
        ESP_LOGW(TAG, "WiFi config not found, starting setup mode...");
        light_node_run_setup_mode();
    ...
    wifi_manager_register_connection_cb(light_node_wifi_connection_cb, NULL);
    ...
    // [Step 5/7] OLED UI
    err = light_node_init_step_oled(&init_ctx, &step_result);
    if (err != ESP_OK) {
    ...
    light_node_framework_register_mqtt_handlers();
    ESP_LOGI(TAG, "Using node_framework handlers");
    
    mqtt_manager_register_connection_cb(light_node_mqtt_connection_cb, NULL);
