...
1. **Workflow state** —
   `PGPASSWORD="${PGPASSWORD:-hydro}" psql -h localhost -U hydro -d hydro_dev -w -c "SELECT workflow_phase, payload->>'workflow' AS workflow, updated_at FROM zone_workflow_state WHERE zone_id=$ARGUMENTS"`

2. **Последние 10 zone_events** —
   `PGPASSWORD="${PGPASSWORD:-hydro}" psql -h localhost -U hydro -d hydro_dev -w -c "SELECT type, payload_json::text, created_at FROM zone_events WHERE zone_id=$ARGUMENTS ORDER BY created_at DESC LIMIT 10"`

3. **Active automation intents** —
   `PGPASSWORD="${PGPASSWORD:-hydro}" psql -h localhost -U hydro -d hydro_dev -w -c "SELECT id, intent_type, status, idempotency_key, created_at FROM zone_automation_intents WHERE zone_id=$ARGUMENTS AND status IN ('pending','running') ORDER BY created_at DESC"`

4. **AE3 runtime state** — `curl -s http://localhost:9405/zones/$ARGUMENTS/state | jq .`

5. **Последние 5 команд** —
   `PGPASSWORD="${PGPASSWORD:-hydro}" psql -h localhost -U hydro -d hydro_dev -w -c "SELECT cmd, status, node_uid, channel, created_at FROM commands WHERE zone_id=$ARGUMENTS ORDER BY created_at DESC LIMIT 5"`
