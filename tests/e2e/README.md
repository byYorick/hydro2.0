...
## Строгие проверки раннера (опционально)

Переменные окружения для сценариев, где нельзя молча подставлять fallback:

| Переменная | Эффект |
|------------|--------|
| `E2E_STRICT_DOCKER_CONTAINER_RESOLVE=1` | Если `docker ps` не нашёл контейнер по labels compose, **не** использовать имя вида `{project}-{service}-1`, а завершить шаг с ошибкой. |
| `E2E_STRICT_CONDITION_ATTRS=1` | В YAML `condition` обращение к несуществующему пути (`context.typo`) даёт ошибку вместо «ложного» `None`. |

По умолчанию поведение прежнее (совместимость со старыми сценариями).
