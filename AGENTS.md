...
- `tests/e2e/AGENTS.md` — локальные правила каталога `tests/e2e/`.
...
- Перед запуском **Playwright** E2E сначала дать пользователю краткие рекомендации по экономичному запуску
  (сузить scope до конкретного spec/`--grep`, анализировать короткий хвост ошибки, включать trace/video/screenshots
  только при падении), и лишь затем запускать тесты.
- Перед запуском **realhw YAML** на физической `test_node` следовать
  `doc_ai/13_TESTING/REALHW_TEST_NODE_AGENT_GUIDE.md`: узкий `--set` / `E2E_SCENARIO_INCLUDE_REGEX`,
  MQTT **1884**, без node-sim, не `--set=full` «на всякий случай».
